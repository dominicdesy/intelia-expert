...
def get_user_profile(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            logger.info(
                f"✅ User profile loaded: production_type={profile['production_type']}, category={profile['category']}, country={profile['country']}"
            )
            return profile
...
def build_personalized_system_prompt(
    base_prompt: str, user_profile: Dict[str, Any]
) -> str:
    """
    ...
    if country == "US":
        ...
        personalization += (
            "- Example: 'The bird weighs 5.2 lb' or 'Set temperature to 95°F'\n"
        )
        ...
        personalization += (
            "- Provide distances/lengths in meters (m) and centimeters (cm)\n"
        )
        personalization += "- Provide volumes in liters (L) and milliliters (mL)\n"
        personalization += (
            "- Example: 'The bird weighs 2.4 kg' or 'Set temperature to 35°C'\n"
        )
        personalization += (
            "- DO NOT use Imperial units unless specifically requested by the user\n"
        )
    ...
    if "broiler" in production_types and "layer" not in production_types:
        ...
        personalization += (
            "- Emphasize growth performance, feed efficiency, and processing quality\n"
        )
        ...
        personalization += (
            "- If a topic applies to only one type, state this explicitly\n"
        )
    ...
    if category == "health_veterinary":
        ...
        personalization += (
            "- Provide DETAILED technical explanations with scientific terminology\n"
        )
        ...
        personalization += (
            "- Discuss pathophysiology, treatment protocols, and therapeutic options\n"
        )
        personalization += (
            "- Use medical terminology (this user has veterinary training)\n"
        )
        ...
        personalization += (
            "- Focus on day-to-day farm management and real-world problem-solving\n"
        )
        personalization += "- Use CLEAR LANGUAGE without excessive technical jargon\n"
        personalization += (
            "- Include specific steps and 'what to look for' indicators\n"
        )
        personalization += (
            "- When discussing health issues, explain WHEN to call a veterinarian\n"
        )
        personalization += (
            "- Focus on prevention, early detection, and practical interventions\n"
        )
        ...
        personalization += "Response Style:\n"
        personalization += (
            "- Provide detailed nutritional analysis and formulation guidance\n"
        )
        personalization += (
            "- Include specific nutrient requirements and formulation strategies\n"
        )
        ...
        personalization += (
            "- Include cost-benefit considerations for feed formulation decisions\n"
        )
        ...
        personalization += "Response Style:\n"
        personalization += (
            "- Provide strategic insights with performance data and KPIs\n"
        )
        personalization += "- Include cost-benefit analysis and ROI considerations\n"
        personalization += (
            "- Focus on decision-making support and performance optimization\n"
        )
        ...
        personalization += "Response Style:\n"
        personalization += (
            "- Focus on breeder management, egg quality, fertility, and hatchability\n"
        )
        personalization += "- Include incubation parameters, embryo development, and hatch window optimization\n"
        personalization += (
            "- Discuss genetic selection, flock management, and breeder nutrition\n"
        )
        personalization += (
            "- Reference breeder company guidelines and hatchery best practices\n"
        )
        ...
        personalization += "Response Style:\n"
        personalization += (
            "- Focus on slaughter operations, meat quality, and food safety\n"
        )
        personalization += "- Include processing yields, carcass quality, shelf life, and product quality\n"
        personalization += (
            "- Discuss regulatory compliance, HACCP protocols, and quality control\n"
        )
        personalization += (
            "- Reference food safety standards and processing best practices\n"
        )
        ...
        personalization += (
            "- Include automation, sensors, IoT, and data management systems\n"
        )
        personalization += "- Discuss ROI, equipment selection criteria, and maintenance requirements\n"
        personalization += (
            "- Reference technical standards and equipment performance metrics\n"
        )
        ...
        personalization += (
            "- Explain technical concepts clearly with practical context\n"
        )
    ...
    logger.info(
        f"✅ System prompt personalized (production_type={production_types}, category={category})"
    )
...
def build_weaviate_filter(user_profile: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not production_types:
        ...
        logger.info(
            "ℹ️ No production_type filter applied (user has no production type set)"
        )
        return None
    ...
    if len(types_to_include) == 1:
        ...
        weaviate_filter = {"operator": "Or", "operands": operands}
