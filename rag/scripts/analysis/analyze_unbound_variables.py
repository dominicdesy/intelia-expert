...
print(
    f"  Conditional imports: {len(categories['conditional_imports'])} (FALSE POSITIVE)"
)
...
print(
    f"  [OK] Definite false positives: {false_positives} ({false_positives/len(parsed_errors)*100:.1f}%)"
)
print(
    f"  [?]  Likely OK (API routes):   {likely_ok} ({likely_ok/len(parsed_errors)*100:.1f}%)"
)
print(
    f"  [!]  Need manual review:       {real_suspects} ({real_suspects/len(parsed_errors)*100:.1f}%)"
)
print()
...
print("[OK] Real suspects saved to: unbound_vars_suspects.txt")
