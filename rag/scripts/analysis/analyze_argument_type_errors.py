    ...
    if " - error: " in line and line.startswith("  "):
        ...
        if len(parts) == 2:
            ...
            if len(loc_parts) == 3:
                file_path = (
                    loc_parts[0]
                    .replace("c:\\intelia_gpt\\intelia-expert\\llm\\", "")
                    .replace("/", "\\")
                )
                try:
...
print(
    "These are the most dangerous - None causes AttributeError/TypeError at runtime\n"
)
...
print(
    f"  [HIGH] None to type:           {len(categories['none_to_type'])} (REVIEW ALL)"
)
print(
    f"  [MED]  Wrong type:              {len(categories['wrong_type'])} (REVIEW SAMPLE)"
)
print(
    f"  [MED]  LanguageDetectionResult: {len(categories['language_detection'])} (SPECIFIC ISSUE)"
)
print(
    f"  [LOW]  Missing protocol:        {len(categories['missing_protocol'])} (LIKELY OK)"
)
...
if len(parsed_errors) > 0:
    ...
    print(
        f"  HIGH RISK (None errors):  {high_risk} ({high_risk/len(parsed_errors)*100:.1f}%)"
    )
    print(
        f"  MEDIUM RISK:              {med_risk} ({med_risk/len(parsed_errors)*100:.1f}%)"
    )
    print(
        f"  LOW RISK:                 {low_risk} ({low_risk/len(parsed_errors)*100:.1f}%)"
    )
    print()
else:
