...
def analyze_faithfulness_issues(json_path: str) -> None:
    ...
    sorted_cases = sorted(
        test_cases, key=lambda x: x.get("scores", {}).get("faithfulness", 1.0)
    )

    # Statistiques
    low_faithfulness = [
        c for c in test_cases if c.get("scores", {}).get("faithfulness", 1.0) < 0.5
    ]
    medium_faithfulness = [
        c
        for c in test_cases
        if 0.5 <= c.get("scores", {}).get("faithfulness", 1.0) < 0.8
    ]
    high_faithfulness = [
        c for c in test_cases if c.get("scores", {}).get("faithfulness", 1.0) >= 0.8
    ]
    ...
    problematic_cases = [
        c for c in sorted_cases if c.get("scores", {}).get("faithfulness", 1.0) < 0.8
    ]
    ...
    if generic_answers:
        print(
            f"📌 Pattern 1: Réponses basées sur connaissances générales ({len(generic_answers)} cas)"
        )
    ...
    for case in problematic_cases:
        ...
        if len(unique_numbers) > 2:  # Plus de 2 nombres uniques
            hallucination_markers.append(
                {"case": case, "unique_numbers": unique_numbers}
            )

    if hallucination_markers:
        print(
            f"📌 Pattern 2: Ajout de chiffres non présents dans le contexte ({len(hallucination_markers)} cas)"
        )
    ...
    if low_recall_cases:
        ...
        print(
            "   → Le contexte récupéré ne contient pas toutes les informations nécessaires"
        )
        print()
    ...
    if len(generic_answers) > len(problematic_cases) * 0.3:
        print(
            "1. ⚠️  PRIORITÉ HAUTE: Renforcer les prompts pour éviter l'usage de connaissances générales"
        )
