    ...
    queries_supplement.append(
        {
            ...
            "domain": domain,
            "query_type": qtype,
            "expected_entities": ["breed", "age_days"],
            "expected_context": (
                "performance_issue" if "compare" not in q.lower() else "comparison"
            ),
    ...
    queries_supplement.append(
        {
            ...
            "domain": domain,
            "query_type": qtype,
            "expected_entities": ["production_phase"],
...
data["metadata"][
    "description"
] = f"{len(data['queries'])} realistic test queries covering all system features"
...
print(
    f"   - Production: {sum(1 for q in data['queries'] if q['domain'] == 'production')}"
)
print(f"   - Health: {sum(1 for q in data['queries'] if q['domain'] == 'health')}")
print(
    f"   - Nutrition: {sum(1 for q in data['queries'] if q['domain'] == 'nutrition')}"
)
print(
    f"   - Environment: {sum(1 for q in data['queries'] if q['domain'] == 'environment')}"
)
print(
    f"   - Management: {sum(1 for q in data['queries'] if q['domain'] == 'management')}"
)
