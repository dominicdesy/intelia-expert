...
export default function ProtectedRoute({ 
  ...
  const { user, isAuthenticated, hasHydrated, checkAuth } = useAuthStore()
  ...
  }, [hasHydrated, isChecking, isAuthenticated, user, requiredUserType, adminOnly, router, redirectTo])

  // États d'affichage
  if (!hasHydrated || isChecking) {
    return <ClientOnly fallback={fallback || defaultFallback}>{fallback || defaultFallback}</ClientOnly>
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Votre compte actuel : <span className="font-medium">{user.user_type}</span>
          </p>
          <button
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-2">Accès Administrateur Requis</h1>
          <p className="text-gray-600 mb-4">
