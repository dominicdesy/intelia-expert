...
export const logoutService = {
  async performLogout(user?: any): Promise<void> {
    ...
    const forceRedirect = setTimeout(() => {
      console.log('[LogoutService] TIMEOUT GLOBAL - Redirection forcée après 8 secondes')
    ...
    try {
      ...
      try {
        ...
        const { getSupabaseClient } = await Promise.race([importPromise, importTimeout]) as any
        ...
        console.log('[LogoutService] Deconnexion Supabase terminee avec succes')
      } catch (supabaseError) {
        console.warn('[LogoutService] Erreur/Timeout Supabase (continue quand meme):', supabaseError?.message || supabaseError)
      }
      ...
      try {
        ...
        console.warn('[LogoutService] Erreur lors du nettoyage localStorage:', storageError)
      }
      ...
      try {
        ...
        console.warn('[LogoutService] Erreur reinitialisation store auth:', error)
      }
      console.log('[LogoutService] Etape 5 terminee - Reinitialisation store auth')
      ...
      try {
        ...
        console.warn('[LogoutService] Erreur definition marqueurs logout:', error)
      }
