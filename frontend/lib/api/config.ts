...
export const API_CONFIG = {
  ...
  BASE_URL: process.env.NEXT_PUBLIC_API_BASE ?? (
    process.env.NODE_ENV === 'development' 
      ? '/api/v1' 
      : '/v1'  // En production, votre backend semble utiliser /v1 directement
  ),
  ...
  SUPABASE_COOKIE_NAME: process.env.NEXT_PUBLIC_SUPABASE_COOKIE_NAME ?? 'sb-cdrmjshmkdfwwtsfdvbl-auth-token'
}
