    ...
    <div className="bg-white py-8 px-6 shadow-lg rounded-lg border border-gray-200">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">
          {t("verification.success.canLogin")}
        </p>
      ...
      <div className="text-center">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
          <h4 className="text-sm font-medium text-red-800 mb-2">
            {t("verification.error.possibleCauses")}
          </h4>
          <ul className="text-sm text-red-700 list-disc list-inside space-y-1">
            <li>{t("verification.error.expired")}</li>
            <li>{t("verification.error.alreadyUsed")}</li>
            <li>{t("verification.error.invalid")}</li>
          </ul>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            {t("verification.error.retrySignup")}
          </Link>
      ...
      <div className="text-center">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <h4 className="text-sm font-medium text-blue-800 mb-2">
            {t("verification.pending.nextSteps")}
          </h4>
          <ol className="text-sm text-blue-700 list-decimal list-inside space-y-1">
            <li>{t("verification.pending.step1")}</li>
            <li>{t("verification.pending.step2")}</li>
            <li>{t("verification.pending.step3")}</li>
          </ol>
        </div>
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status === 400) {
            setMessage(t("verification.error.invalidOrExpired"));
          } else if (response.status === 409) {
            setMessage(t("verification.error.alreadyVerified"));
          } else {
            setMessage(errorData.detail || t("verification.error.generic"));
          }
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {status === "success" && (
          <VerificationSuccessPage email={email || undefined} />
        )}
        {status === "error" && <VerificationErrorPage error={message} />}
        {status === "pending" && (
          <PendingVerificationPage email={email || undefined} />
        )}
        ...
        <div className="p-3 bg-gray-50 rounded-lg text-center">
          <p className="text-xs text-gray-600">
            {t("verification.supportQuestion")}{" "}
            <button
              type="button"
              onClick={() =>
                window.open(
                  `mailto:${t("contact.supportEmail")}?subject=${encodeURIComponent(t("verification.supportSubject"))}`,
                  "_blank",
                )
              }
      ...
      fallback={
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="animate-spin rounded-full h-8 w-8 border-2 border-blue-600 border-t-transparent rounded-full mx-auto mb-4"></div>
