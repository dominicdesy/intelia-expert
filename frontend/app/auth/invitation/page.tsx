  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        ...
        console.warn('[Countries] API REST Countries bloquée, utilisation du fallback:', err)
...
const ProcessingStatus = ({ result }: { result: ProcessingResult }) => {
  ...
  const getStepText = () => {
    switch (result.step) {
      case 'validation':
        return result.success ? t('invitation.status.tokenValidated') : t('invitation.status.tokenError')
      case 'completion':
        return result.success ? t('invitation.status.accountCreated') : t('invitation.status.accountError')
      default:
...
function InvitationAcceptPageContent() {
      ...
      try {
        ...
        if (hasInvitationInHash || hasInvitationInQuery) {
          ...
          if (!validateResponse.ok) {
            ...
            setProcessingResult({
              success: false,
              step: 'validation',
              message: errorData.detail || t('invitation.errors.tokenValidation'),
              details: errorData
            })
            throw new Error(errorData.detail || t('invitation.errors.tokenValidation'))
          }
        ...
        setTimeout(() => {
          router.push('/auth/login?error=' + encodeURIComponent(
            error instanceof Error ? error.message : t('invitation.errors.generic')
          ))
        }, 4000)
      }
    }
  ...
  const validateForm = (): string[] => {
    ...
    if (!validatePhone(formData.countryCode, formData.areaCode, formData.phoneNumber, formData.country, countryCodeMap)) {
      ...
      if (hasUserEnteredPhoneData) {
        const missingFields = []
        if (!formData.countryCode.trim()) missingFields.push(t('profile.countryCode'))
        if (!formData.areaCode.trim()) missingFields.push(t('profile.areaCode'))
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      if (!requestBody.firstName) clientValidationErrors.push(t('validation.required.firstName'))
      if (!requestBody.lastName) clientValidationErrors.push(t('validation.required.lastName'))
      if (!requestBody.fullName) clientValidationErrors.push(t('validation.required.fullName'))
      if (!requestBody.email) clientValidationErrors.push(t('validation.required.email'))
      if (!requestBody.country) clientValidationErrors.push(t('validation.required.country'))
      if (!requestBody.password) clientValidationErrors.push(t('validation.required.password'))
      if (!requestBody.access_token) clientValidationErrors.push(t('validation.required.accessToken'))
      
      if (clientValidationErrors.length > 0) {
        console.error('[InvitationAccept] Erreurs validation client:', clientValidationErrors)
        throw new Error(t('validation.invalidData') + ': ' + clientValidationErrors.join(', '))
      }
      ...
      if (!completeResponse.ok) {
        ...
        try {
          ...
          errorData = { detail: `${t('error.generic')} ${completeResponse.status}: ${completeResponse.statusText}` }
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="text-center text-sm text-gray-600 mb-8">
          {status === 'set-password' ? t('invitation.completeProfile') : t('invitation.finalizingInvitation')}
        </p>
      </div>
            ...
            <div>
              ...
              {userInfo && (
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h3 className="font-medium text-blue-900 mb-2">{t('invitation.validatedSuccess')}</h3>
                  <div className="text-sm text-blue-800 space-y-1">
                    <p><strong>{t('profile.email')}:</strong> {userInfo.email}</p>
                    {userInfo.inviterName && (
                      <p><strong>{t('invitation.invitedBy')}:</strong> {userInfo.inviterName}</p>
                    )}
                    {userInfo.personalMessage && (
                      <div className="mt-2 p-2 bg-white rounded border">
                        <p className="text-xs text-gray-600 mb-1">{t('invitation.personalMessage')}:</p>
              ...
              <div className="space-y-6">
                ...
                <div className="border-b border-gray-200 pb-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                    />
                    <p className="mt-1 text-xs text-gray-500">{t('invitation.emailFromInvitation')}</p>
                  </div>
                    ...
                    {countriesLoading ? (
                      <div className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 bg-gray-50">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-gray-600">{t('countries.loading')}</span>
                        </div>
                      </div>
                    ) : (
                      <select
                        ...
                        disabled={isProcessing}
                      >
                        <option value="">{t('placeholder.countrySelect')}</option>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{t('profile.countryCode')}</label>
                        <input
                          type="text"
                          placeholder="+1"
                      ...
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{t('profile.areaCode')}</label>
                        <input
                          type="text"
                          placeholder="514"
                      ...
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{t('profile.phoneNumber')}</label>
                        <input
                          type="text"
                          placeholder="1234567"
                ...
                {/* Section Entreprise */}
                <div className="border-b border-gray-200 pb-6">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">{t('profile.company')}</h3>
                ...
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">{t('profile.security')}</h3>
                  ...
                  <div>
                    ...
                    {formData.password && (
                      <div className="mt-3 bg-gray-50 rounded-lg p-3">
                        <h5 className="text-sm font-medium text-gray-900 mb-2">{t('validation.password.requirements')}:</h5>
