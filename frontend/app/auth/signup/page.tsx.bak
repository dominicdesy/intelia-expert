...
export default function SignupPage() {
  // 🔄 NOUVEAU : Utiliser le store backend
  ...
  useEffect(() => {
    console.log('🚨 FICHIER UTILISÉ: Page signup AVEC Backend Store')
    console.log('=== DEBUG: Page Signup Backend ===')
    console.log('Backend URL:', process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000')
    console.log('Store utilisé: Backend API (useAuthStore)')
  ...
  // Nettoyer les erreurs quand on change les données
  useEffect(() => {
    if (error) {
      clearError()
    }
  ...
  const handleSubmit = async () => {
    console.log('🚨 DEBUG: handleSubmit BACKEND appelé')
    console.log('=== DEBUG: handleSubmit Backend ===')
    ...
    try {
      console.log('🔍 DÉBUT: Création compte via Backend API')
      ...
      console.log('🔍 Appel register() du store backend...')
      await register(formData.email.trim(), formData.password, userData)
      
      console.log('✅ Inscription réussie via backend!')
      ...
      // Redirection sera gérée par le store ou AuthProvider
      
    } catch (error: any) {
      console.error('❌ Erreur inscription backend:', error)
      
      // L'erreur est déjà gérée par le store, mais on peut ajouter des logs
      ...
      // Les erreurs sont maintenant gérées par le store
    }
  }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-lg sm:rounded-lg sm:px-10">
          ...
          <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-xs">
            <div className="font-bold text-green-800 mb-1">🔍 DEBUG - Page Signup Backend</div>
            <div className="text-green-700">
              ...
              • Store: Backend API (useAuthStore)<br/>
