...
export default function SignupPage() {
  ...
  useEffect(() => {
    ...
    console.log('Type de page: Page signup SIMPLE (sans AuthStore)')
    
    // Test si on a accès aux stores
    try {
      const hasAuthStore = typeof window !== 'undefined' && 'useAuthStore' in window
      console.log('AuthStore disponible?', hasAuthStore)
    } catch (e) {
      console.log('Erreur test AuthStore:', e)
    }
  }, [])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        // @ts-ignore - Test de l'existence de Supabase
        if (typeof window !== 'undefined' && window.supabase) {
          console.log('⚠️ Supabase client détecté dans window')
        }
      } catch (e) {
        console.log('Pas de Supabase global:', e)
      }
      ...
      console.log('🔍 Création compte avec standards sécurisés:', {
        email: formData.email,
        firstName: formData.firstName,
        lastName: formData.lastName,
        passwordStrength: 4 - validatePassword(formData.password).length
      })
      
      console.log('🔍 Simulation en cours... (2 secondes)')
      await new Promise(resolve => setTimeout(resolve, 2000))
      
      console.log('✅ Simulation terminée - Redirection vers verify-email')
      
      // Redirection vers la page de vérification email ou directement vers le chat
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-lg sm:rounded-lg sm:px-10">
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs">
            <div className="font-bold text-blue-800 mb-1">🔍 DEBUG - Page Signup Simple</div>
            <div className="text-blue-700">
              • Page: {typeof window !== 'undefined' ? window.location.pathname : 'N/A'}<br/>
              • Supabase: {process.env.NEXT_PUBLIC_SUPABASE_URL ? '✓' : '❌'}<br/>
              • Type: Formulaire simple sans AuthStore<br/>
              • Appel API: Commenté (ligne 206-215)
            </div>
          </div>
