// hooks/useAuthStore.ts - FIX PERMANENT POUR SUPER_ADMIN
...
const fetchUserProfileFromBackend = async (session: any): Promise<Partial<User> | null> => {
  try {
    ...
    if (!token) {
      console.warn('⚠️ Pas de token pour récupérer le profil backend')
      return null
    }
    ...
    if (!response.ok) {
      console.warn('⚠️ Erreur API backend profil:', response.status)
      return null
    }
    ...
    console.warn('⚠️ Erreur récupération profil backend:', error)
    return null
  }
}
    ...
    const loadUser = async () => {
      try {
        ...
        if (session?.user && isMountedRef.current && !isLoggingOutRef.current) {
          ...
          // 🆕 RÉCUPÉRER LE PROFIL DEPUIS VOTRE BACKEND
          const backendProfile = await fetchUserProfileFromBackend(session)
          
          // 🎯 CONSTRUIRE L'OBJET USER EN COMBINANT SUPABASE + BACKEND
          const userData: User = {
            id: session.user.id,
            ...
            // 🔥 PRIORITÉ AU BACKEND POUR USER_TYPE (fix super_admin)
            user_type: backendProfile?.user_type || session.user.user_metadata?.role || 'producer',
          ...
          console.log('🔍 User final:', {
            ...
            backend_user_type: backendProfile?.user_type,
            supabase_role: session.user.user_metadata?.role
          })
