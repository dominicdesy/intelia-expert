...
export default function ChatInterface() {
  ...
  const redirectToLogin = useCallback((reason?: string) => {
    ...
    console.log('Redirection vers login:', reason || 'Session expirée')
    router.push('/')
  }, [router])
    ...
    if (hasHydrated && isAuthenticated === false && !isLoading && isMountedRef.current) {
      ...
      if (recentLogout) {
        ...
        if (now - logoutTime < 5000) {
          console.log('Déconnexion récente détectée, pas de redirection automatique')
      ...
      try {
        ...
        if (comment && comment.trim()) {
          try {
            ...
            console.warn('Commentaire non envoyé:', commentError)
          }
        }
      } catch (feedbackError) {
