        ...
        try {
          ...
          switch (event.type) {
            case "agent_start":
              ...
              console.log('[apiService] Agent démarré:', agentStartEvent.complexity, 'sous-requêtes:', agentStartEvent.sub_queries_count);
              agentMetadata.complexity = agentStartEvent.complexity || 'simple';
              agentMetadata.sub_queries_count = agentStartEvent.sub_queries_count || 0;
              callbacks?.onAgentStart?.(agentStartEvent.complexity, agentStartEvent.sub_queries_count);
              break;
              
            case "agent_thinking":
              const agentThinkingEvent = event as AgentThinkingEvent;
              console.log('[apiService] Agent réflexion:', agentThinkingEvent.decisions?.length || 0, 'décisions');
              if (agentThinkingEvent.decisions && Array.isArray(agentThinkingEvent.decisions)) {
            ...
            case "agent_progress":
              ...
              console.log('[apiService] Agent progression:', agentProgressEvent.step, agentProgressEvent.progress + '%');
              callbacks?.onAgentProgress?.(agentProgressEvent.step, agentProgressEvent.progress);
              break;
              
            case "agent_end":
              const agentEndEvent = event as AgentEndEvent;
              console.log('[apiService] Agent terminé:', agentEndEvent.synthesis_method, 'sources:', agentEndEvent.sources_used);
              agentMetadata.synthesis_method = agentEndEvent.synthesis_method || 'direct';
              agentMetadata.sources_used = agentEndEvent.sources_used || 0;
              callbacks?.onAgentEnd?.(agentEndEvent.synthesis_method, agentEndEvent.sources_used);
              break;
              
            case "agent_error":
              const agentErrorEvent = event as AgentErrorEvent;
              console.error('[apiService] Erreur Agent:', agentErrorEvent.error);
            ...
            case "proactive_followup":
              ...
              if (followupEvent.suggestion) {
                console.log('[apiService] Relance proactive reçue:', followupEvent.suggestion);
            ...
            default:
              console.log('[apiService] Événement SSE non géré:', (event as any).type, event);
          }
