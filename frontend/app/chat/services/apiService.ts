...
interface EnhancedAIResponse {
  ...
  type?: 'answer' | 'clarification' | 'partial_answer'
  questions?: string[]
  source?: string
...
/**
 * 🔧 FONCTION PRINCIPALE ADAPTÉE : Compatible DialogueManager + garde compatibilité
 */
export const generateAIResponse = async (
  ...
  try {
    ...
    // 🔧 CORRECTION CRITIQUE : Préserver le format selon le type de réponse
...
export const generateAIResponsePublic = async (
  ...
  try {
    ...
    // 🔧 MÊME CORRECTION que la version auth
    const processedData: EnhancedAIResponse = {
      conversation_id: finalConversationId,
      language: language,
      timestamp: new Date().toISOString(),
...
export const debugEnhancedAPI = () => {
  ...
  console.log('  ✅ PRÉSERVATION format partial_answer + general_answer')
...
export const logEnhancedAPIInfo = () => {
  ...
  console.log('  - 🚀 Session ID via header X-Session-ID')
  console.log('  - 🚀 Body simplifié: { question }')
  console.log('  - 🚀 Support type: clarification/answer/partial_answer')
  ...
  console.log('  - ✅ Support extractAnswerAndSources() natif')
  console.groupEnd()
}
