              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white border border-gray-200 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-800">Temps Médian</span>
                  </div>
                  <p className="text-lg font-semibold text-blue-900">
                    {performanceStats.median_response_time?.toFixed(1) || '0.0'}s
                  </p>
                ...
                <div className="bg-white border border-gray-200 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-green-800">Min/Max</span>
                  </div>
                  <p className="text-lg font-semibold text-green-900">
                    {performanceStats.min_response_time?.toFixed(1) || '0.0'}s / {performanceStats.max_response_time?.toFixed(1) || '0.0'}s
                  </p>
              ...
              {performanceStats.avg_response_time && (
                <div className="mt-4">
                  <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                    ...
                    <span>
                      {performanceStats.avg_response_time < 2 ? 'Excellent' :
                       performanceStats.avg_response_time < 5 ? 'Bon' :
                       performanceStats.avg_response_time < 10 ? 'Acceptable' : 'À améliorer'}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                    <div 
                      className={`h-2 rounded-full transition-all duration-500 ${
                        performanceStats.avg_response_time < 2 ? 'bg-green-500' :
                        performanceStats.avg_response_time < 5 ? 'bg-blue-500' :
                        performanceStats.avg_response_time < 10 ? 'bg-yellow-500' :
                        'bg-red-500'
                      }`}
                      style={{ 
