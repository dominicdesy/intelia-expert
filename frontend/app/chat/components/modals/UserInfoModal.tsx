...
const PasswordStrengthIndicator: React.FC<{ password: string }> = ({ password }) => {
  ...
  const requirements = [
    { test: password.length >= 8, label: t('profile.passwordRequirements8') },
    { test: /[A-Z]/.test(password), label: t('profile.passwordRequirementsUpper') },
    { test: /[a-z]/.test(password), label: t('profile.passwordRequirementsLower') },
    { test: /\d/.test(password), label: t('profile.passwordRequirementsNumber') },
    { test: /[!@#$%^&*(),.?":{}|<>]/.test(password), label: t('profile.passwordRequirementsSpecial') }
  ]
...
export const UserInfoModal: React.FC<UserInfoModalProps> = ({ user, onClose }) => {
  ...
  const validatePassword = useCallback((password: string): string[] => {
    ...
    if (password.length < 8) {
      errors.push(t('form.passwordMinLength'))
    }
    if (!/[A-Z]/.test(password)) {
      errors.push(t('form.passwordUppercase'))
    }
    if (!/[a-z]/.test(password)) {
      errors.push(t('form.passwordLowercase'))
    }
    if (!/\d/.test(password)) {
      errors.push(t('form.passwordNumber'))
    }
    if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
      errors.push(t('form.passwordSpecial'))
    }
  ...
  const handleProfileSave = useCallback(async () => {
    ...
    try {
      ...
      // API call direct sans passer par le store auth
      ...
      console.log(t('success.profileUpdated'))
      ...
      if (isMountedRef.current) {
        ...
        setFormErrors([t('common.unexpectedError')])
      }
    } finally {
      if (isMountedRef.current) {
        setIsLoading(false)
  ...
  }, [passwordData, validatePassword, user?.email, handleClose, isLoading, t])
