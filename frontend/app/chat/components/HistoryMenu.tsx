...
// 🛠️ CORRECTION MAJEURE : Enveloppement avec React.memo pour éviter les re-rendus
export const HistoryMenu = React.memo(() => {
  ...
  // 🔧 CORRECTION : Éviter la boucle infinie dans handleToggle avec useCallback
  ...
  // 🛠️ CORRECTION : Mémoisation des handlers pour éviter les re-créations
  ...
  const handleConversationClick = useCallback(async (conv: Conversation, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    await loadConversation(conv.id) // charge la conversation complète (avec messages)
    setIsOpen(false)
  }, [loadConversation])
  ...
  // 🛠️ CORRECTION : Mémoisation de la fonction de formatage
  ...
  // 🛠️ CORRECTION : Mémoisation du calcul du total de conversations
  ...
  // 🛠️ CORRECTION CRITIQUE : Réduire la fréquence des logs de debug
  // Utilisez useMemo pour éviter que les logs se déclenchent à chaque re-rendu
  const debugState = useMemo(() => {
    ...
    // 🔧 LOGS CONDITIONNELS : Ne loguer que si l'état a vraiment changé
    console.log('[HistoryMenu] Render state:', state)
    
    return state
  }, [isOpen, totalConversations, conversationGroups.length, isLoadingHistory, user])

  // 🛠️ CORRECTION : Mémoisation du rendu des conversations pour optimiser les performances
  ...
  // 🛠️ CORRECTION : Mémoisation du badge de notification
  ...
  // 🛠️ CORRECTION : Callback pour fermer le menu
  ...
  return (
    <div className="relative header-icon-container">
      ...
      {/* 🔧 CORRECTION CRITIQUE : S'assurer que le menu s'affiche quand isOpen=true */}
      {isOpen && (
        <>
...
// 🛠️ CORRECTION : Ajouter le displayName pour le debugging
