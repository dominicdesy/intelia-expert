      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error("Cette conversation partagée n'existe pas ou a été supprimée");
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm text-blue-900 font-medium">
                Conversation partagée par {share_info.shared_by}
              </p>
              <div className="flex items-center gap-4 mt-2 text-xs text-blue-700">
                <div className="flex items-center gap-1">
                  <Eye size={14} />
                  <span>{share_info.view_count} vue{share_info.view_count > 1 ? "s" : ""}</span>
                </div>
                {share_info.expires_at && (
                  <div className="flex items-center gap-1">
                    <Clock size={14} />
                    <span>
                      Expire le {new Date(share_info.expires_at).toLocaleDateString("fr-FR")}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          <p>
            Cette conversation a été générée par{" "}
            <a
          ...
          {share_info.anonymized && (
            <p className="mt-1 text-xs text-gray-400">
              Les données personnelles ont été anonymisées pour protéger la vie privée
            </p>
          )}
        </div>
      </div>
