...
class AdvancedResponseGuardrails:
    ...
    async def verify_response(self, query: str, response: str, 
        ...
        try:
            ...
            consistency_task = self._check_domain_consistency(response, query, intent_result)
            ...
            if evidence_score < 0.3:
                ...
                corrections.append("Reformuler en s'appuyant davantage sur les documents fournis")
            ...
            is_valid = self._make_validation_decision(
                evidence_score, hallucination_risk, consistency_score, 
    ...
    async def _check_evidence_support(self, response: str, 
                                    context_docs: List[Dict]) -> Tuple[float, Dict]:
        ...
        try:
            ...
            evidence_score = supported_claims / total_claims if total_claims > 0 else 0.5
            
            # Bonus pour indicateurs explicites d'évidence
            evidence_indicators_found = sum(1 for pattern in self.evidence_indicators 
                                          if re.search(pattern, response.lower()))
            
            if evidence_indicators_found > 0:
                evidence_score = min(1.0, evidence_score + 0.1 * evidence_indicators_found)
    ...
    async def _detect_hallucination_risk(self, response: str, 
                                       context_docs: List[Dict]) -> Tuple[float, Dict]:
        ...
        try:
            ...
            unsupported_statements = await self._find_unsupported_statements(response, context_docs)
    ...
    async def _check_domain_consistency(self, response: str, query: str, 
                                      intent_result) -> Tuple[float, List[str]]:
        ...
        try:
            ...
            if total_domain_words > 0:
                domain_score = min(1.0, 0.3 + (domain_words_found / total_domain_words) * 0.7)
                ...
                for entity_type in ['line', 'species', 'phase']:
                    if entity_type in entities:
                        ...
                        if entity_value in response.lower():
                            ...
                            issues.append(f"Entité {entity_type} '{entities[entity_type]}' non mentionnée")
            ...
            inconsistent_terms = [
                "crypto", "bitcoin", "politique", "football", "cinéma"
            ]
    ...
    async def _verify_factual_claims(self, response: str, 
                                   context_docs: List[Dict]) -> Tuple[float, List[str]]:
        ...
        try:
            ...
            qualitative_claims = [
                "recommandé", "optimal", "idéal", "maximum", "minimum",
                "meilleur", "préférable", "conseillé", "éviter"
            ]
            
            for claim_word in qualitative_claims:
                if claim_word in response.lower():
                    total_claims += 1
                    if await self._verify_qualitative_claim(claim_word, response, context_docs):
                        verified_claims += 1
                    else:
                        issues.append(f"Affirmation qualitative non supportée: {claim_word}")
    ...
    async def _find_unsupported_statements(self, response: str, 
    ...
    async def _verify_numeric_claim(self, numeric_text: str, 
                                  context_docs: List[Dict]) -> bool:
    ...
    async def _verify_specific_value(self, value: str, unit: str, 
                                   context_docs: List[Dict]) -> bool:
    ...
    async def _verify_qualitative_claim(self, claim_word: str, response: str, 
                                      context_docs: List[Dict]) -> bool:
    ...
    def _calculate_global_confidence(self, evidence_score: float, 
                                   hallucination_risk: float,
                                   consistency_score: float,
                                   factual_accuracy: float) -> float:
    ...
    def _make_validation_decision(self, evidence_score: float, 
                                hallucination_risk: float,
                                consistency_score: float,
                                violation_count: int,
                                warning_count: int) -> bool:
        ...
        try:
            if self.verification_level == VerificationLevel.MINIMAL:
                ...
                return (violation_count == 0 and 
                       evidence_score >= 0.4 and 
    ...
    async def quick_verify(self, response: str, context_docs: List[Dict]) -> bool:
        ...
        try:
            ...
            return overlap_ratio >= 0.3  # Au moins 30% des docs doivent avoir un overlap
    ...
    def get_guardrails_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_thresholds": {
                ...
                "standard": {"violations": 0, "evidence_min": 0.4, "hallucination_max": 0.7},
                "strict": {"violations": 0, "warnings_max": 2, "evidence_min": 0.6, "hallucination_max": 0.5},
                "critical": {"violations": 0, "warnings_max": 1, "evidence_min": 0.8, "hallucination_max": 0.3}
            }
        }
