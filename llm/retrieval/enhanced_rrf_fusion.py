...
class IntelligentRRFFusion:
    ...
    def __init__(self, redis_client=None, intent_processor=None):
        ...
        self.genetic_line_profiles = {
            "ross_308": {
                "vector_preference": 0.25,  # RÉDUIT: Favorise BM25 pour termes exacts
                "bm25_preference": 0.75,  # AUGMENTÉ: Favorise BM25
                "optimal_rrf_k": 35,  # RÉDUIT: Plus précis pour top results
                "specialization_boost": 2.0,  # AUGMENTÉ: Boost majeur
            },
            "cobb_500": {
                "vector_preference": 0.25,  # Même ajustement
                "bm25_preference": 0.75,
                "optimal_rrf_k": 40,
                "specialization_boost": 1.8,
            },
            "hubbard": {
                "vector_preference": 0.30,
                "bm25_preference": 0.70,
                "optimal_rrf_k": 40,
                "specialization_boost": 1.6,
            },
            "isa_brown": {
                "vector_preference": 0.35,
        ...
        self.genetic_boost_factor = float(os.getenv("GENETIC_LINE_BOOST_FACTOR", "2.5"))
        self.genetic_priority_enabled = (
            os.getenv("GENETIC_LINE_PRIORITY_ENABLED", "true").lower() == "true"
        )
    ...
    async def _analyze_query_context(self, query_context: Dict, intent_result) -> Dict:
        ...
        # Detection métriques performance
    ...
    async def _weighted_rrf_fusion(
        ...
        """Fusion RRF pondérée avec boost spécialisé aviculture - MODIFIÉE AVEC BOOST LIGNÉE"""
        ...
        for content_key, data in all_docs.items():
            ...
            # AJOUT: Boost génétique prioritaire
            doc = data["doc"].copy()
            genetic_boost = self._calculate_poultry_boost(doc, context)
            if genetic_boost > 1.5:  # Si boost significatif de lignée
                final_score *= genetic_boost
            ...
            # AJOUT: Marquer pour debugging
            if genetic_boost > 1.5:
                doc["metadata"]["genetic_boost_applied"] = genetic_boost
    ...
    def _calculate_poultry_boost(self, doc: Dict, context: Dict) -> float:
        ...
        # BOOST PRIORITAIRE: Lignée génétique exacte
        query_genetic = context.get("genetic_line", "")
        if query_genetic and self.genetic_priority_enabled:
            # Conversion pour matching
            query_normalized = query_genetic.replace("_", " ")

            # Boost majeur si lignée métadonnée correspond
            if genetic_line_doc and (
                query_normalized in genetic_line_doc
                or genetic_line_doc in query_normalized
            ):
                boost *= self.genetic_boost_factor  # BOOST MAJEUR (2.5x par défaut)

            # Boost moyen si lignée dans contenu
            elif query_genetic in content or query_normalized in content:
                boost *= self.genetic_boost_factor * 0.8  # 2.0x par défaut
        ...
        return min(3.0, boost)  # Limite le boost maximum à 3.0x
    ...
    def _detect_genetic_line(self, query_lower: str) -> str:
        ...
        # DÉTECTION DE LIGNÉE VIA BREEDS_REGISTRY
        # Utilise la source unique de vérité (intents.json) pour supporter les 51 races

        if not self.breeds_registry:
            logger.error(
                "breeds_registry non initialisé - Détection de lignée impossible. "
                "Vérifiez le chargement de intents.json dans __init__."
            )
            return None

        try:
            # Obtenir toutes les races du registry (51 races)
            all_breeds = self.breeds_registry.get_all_breeds()

            # Trier par longueur décroissante pour matcher les plus spécifiques d'abord
            # Ex: "ross 308" avant "ross" pour éviter faux positifs
            sorted_breeds = sorted(all_breeds, key=len, reverse=True)
            ...
            for breed in sorted_breeds:
                ...
                # 1. Vérifier le nom normalisé
                normalized_lower = normalized.lower()
                if normalized_lower in query_lower:
                    logger.debug(f"Lignée détectée (canonique): {normalized}")
                    return normalized.replace(" ", "_").replace("-", "_")

                # 2. Vérifier tous les aliases de cette race
                aliases = self.breeds_registry.get_aliases(normalized)

                # Trier aliases par longueur décroissante aussi
            ...
            logger.debug("Aucune lignée détectée dans la requête")
            return None

        except Exception as e:
            logger.error(
                f"Erreur lors de la détection de lignée via breeds_registry: {e}",
                exc_info=True,
            )
            return None
