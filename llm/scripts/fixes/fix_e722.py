...
files_to_fix = {
    ...
    'scripts/deep_optimization_analysis.py': [130, 180, 230, 272, 308, 345, 394, 426, 468],
    ...
    for line_num in sorted(line_numbers, reverse=True):
        ...
        if idx < len(lines):
            ...
            if re.search(r'^\s*except\s*:\s*$', original):
                ...
                fixed_count += 1
                print(f"FIX {file_path}:{line_num}: {original.strip()} -> {lines[idx].strip()}")
            else:
                print(f"WARN {file_path}:{line_num}: Not a bare except: {original.strip()}")
