...
class EnhancedResponseGenerator:
    ...
    def __init__(
        ...
        # 🧠 CoT Model Selection - Support for advanced reasoning models
        # Default: gpt-4o (balanced performance/cost)
        # Options: gpt-4o-2024-08-06 (better instruction following), o1-preview/o1-mini (native CoT)
...
"""
    ...
    def _add_cot_instruction(self, prompt: str, structured: bool = True, language: str = "en") -> str:
        """
        Ajoute les instructions Chain-of-Thought au prompt (multilingue)

        Args:
            prompt: Prompt de base
            structured: Si True, utilise CoT structuré XML (Phase 2), sinon CoT simple (Phase 1)
            language: Code langue (nl, fr, en, es, etc.) pour adapter les instructions
...
🧠 CHAIN-OF-THOUGHT REASONING - STRUCTUREER JE ANTWOORD:

Structureer je antwoord met de volgende XML-tags om je redenering te tonen:

<thinking>
[Je eerste redenering over de vraag: wat vraagt de gebruiker? welke informatie is relevant? welke aanpak neem ik?]
</thinking>

<analysis>
[Je gedetailleerde stapsgewijze analyse: extractie van gegevens uit de context, berekeningen indien nodig, verificatie van consistentie, identificatie van sleutelinformatie]
</analysis>

<answer>
[Je definitieve antwoord, helder, beknopt en direct op de vraag van de gebruiker - ZONDER XML-tags in deze sectie]
</answer>

⚠️ BELANGRIJK:
- De secties <thinking> en <analysis> laten de gebruiker je redenering zien
- De sectie <answer> bevat het definitieve antwoord normaal geformatteerd (markdown, lijsten, etc.)
- Elke sectie moet substantieel en informatief zijn

📋 VOORBEELD:
<thinking>
De gebruiker vraagt naar hittestress beheer bij vleeskuikens. Ik moet kijken naar praktische strategieën in de documentatie over ventilatie, water, en klimaatcontrole.
</thinking>

<analysis>
In de context vind ik informatie over:
1. Ventilatie verbetering - ventilatoren en luchtstroom
2. Waterbeschikbaarheid - koel en schoon water
3. Temperatuurcontrole - ideale ranges
Deze drie elementen samen vormen een complete aanpak.
</analysis>

<answer>
Om hittestress bij vleeskuikens te beheersen, zijn drie strategieën essentieel:

- Verbeter de ventilatie door ventilatoren optimaal af te stellen
- Zorg voor constante toegang tot koel, schoon water
- Handhaaf stabiele temperaturen binnen het optimale bereik
</answer>
""",
                "fr": """
...
📋 EXEMPLE:
<thinking>
L'utilisateur demande comment gérer le stress thermique chez les poulets de chair. Je dois chercher des stratégies pratiques dans la documentation sur la ventilation, l'eau et le contrôle climatique.
</thinking>

<analysis>
Dans le contexte je trouve des informations sur:
1. Amélioration de la ventilation - ventilateurs et flux d'air
2. Disponibilité de l'eau - eau fraîche et propre
3. Contrôle de température - plages idéales
Ces trois éléments forment ensemble une approche complète.
</analysis>

<answer>
Pour gérer le stress thermique chez les poulets de chair, trois stratégies sont essentielles:

- Améliorer la ventilation en optimisant les ventilateurs
- Assurer un accès constant à de l'eau fraîche et propre
- Maintenir des températures stables dans la plage optimale
</answer>
""",
                "en": """
...
📋 EXAMPLE:
<thinking>
The user is asking about heat stress management in broilers. I need to look for practical strategies in the documentation about ventilation, water, and climate control.
</thinking>

<analysis>
In the context I find information about:
1. Ventilation improvement - fans and airflow
2. Water availability - cool and clean water
3. Temperature control - ideal ranges
These three elements together form a complete approach.
</analysis>

<answer>
To manage heat stress in broilers, three strategies are essential:

- Improve ventilation by optimizing fan settings
- Ensure constant access to cool, clean water
- Maintain stable temperatures within the optimal range
</answer>
""",
                "es": """

🧠 CHAIN-OF-THOUGHT REASONING - ESTRUCTURA TU RESPUESTA:

Estructura tu respuesta con las siguientes etiquetas XML para mostrar tu razonamiento:

<thinking>
[Tu razonamiento inicial sobre la pregunta: ¿qué pregunta el usuario? ¿qué información es relevante? ¿qué enfoque tomar?]
</thinking>

<analysis>
[Tu análisis detallado paso a paso: extracción de datos del contexto, cálculos si es necesario, verificación de coherencia, identificación de información clave]
</analysis>

<answer>
[Tu respuesta final clara, concisa y directa a la pregunta del usuario - SIN etiquetas XML en esta sección]
</answer>

⚠️ IMPORTANTE:
- Las secciones <thinking> y <analysis> permiten al usuario ver tu razonamiento
- La sección <answer> contiene la respuesta final formateada normalmente (markdown, listas, etc.)
- Cada sección debe ser sustancial e informativa

📋 EJEMPLO:
<thinking>
El usuario pregunta sobre manejo del estrés térmico en pollos de engorde. Debo buscar estrategias prácticas en la documentación sobre ventilación, agua y control climático.
</thinking>

<analysis>
En el contexto encuentro información sobre:
1. Mejora de ventilación - ventiladores y flujo de aire
2. Disponibilidad de agua - agua fresca y limpia
3. Control de temperatura - rangos ideales
Estos tres elementos juntos forman un enfoque completo.
</analysis>

<answer>
Para manejar el estrés térmico en pollos de engorde, tres estrategias son esenciales:

- Mejorar la ventilación optimizando los ventiladores
- Asegurar acceso constante a agua fresca y limpia
- Mantener temperaturas estables dentro del rango óptimo
</answer>
""",
            }

            # Utiliser le template approprié ou fallback sur anglais
            cot_instruction = cot_templates.get(language, cot_templates["en"])
        else:
            # Phase 1: CoT simple - MULTILINGUE
            simple_templates = {
                "nl": "\n\n🧠 AANPAK: Analyseer deze vraag stap voor stap voordat je antwoordt.",
                ...
                "es": "\n\n🧠 ENFOQUE: Analiza esta pregunta paso a paso antes de responder.",
            }
            cot_instruction = simple_templates.get(language, simple_templates["en"])
