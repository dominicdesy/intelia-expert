...
class GenerateRequest(BaseModel):
    ...
    messages: Optional[List[Dict[str, str]]] = Field(
        None, description="Pre-formatted messages (overrides query)"
    )
    ...
    query_type: Optional[str] = Field(
        None, description="Query type (standard, comparative, etc.)"
    )
    context_docs: Optional[List[Dict]] = Field(None, description="Context documents")
    user_category: Optional[str] = Field(
        None,
        description="User expertise level (health_veterinary, farm_operations, etc.)",
    )
    ...
    max_tokens: Optional[int] = Field(
        None, ge=1, le=8000, description="Max tokens (auto-calculated if not provided)"
    )
    ...
    add_disclaimer: bool = Field(
        default=True, description="Add veterinary disclaimer if applicable"
    )
...
class GenerateResponse(BaseModel):
    ...
    calculated_max_tokens: Optional[int] = Field(
        None, description="Auto-calculated max_tokens"
    )
    post_processed: bool = Field(
        default=False, description="Whether post-processing was applied"
    )
    disclaimer_added: bool = Field(
        default=False, description="Whether disclaimer was added"
    )
    cached: bool = Field(
...
class RouteRequest(BaseModel):
    ...
    intent_result: Optional[Dict[str, Any]] = Field(
        None, description="Intent classification result"
    )
...
class PostProcessRequest(BaseModel):
    ...
    add_disclaimer: bool = Field(
        default=True, description="Add disclaimer if applicable"
    )
...
class PostProcessResponse(BaseModel):
    ...
    is_veterinary: bool = Field(
        default=False, description="Whether query was veterinary-related"
    )
