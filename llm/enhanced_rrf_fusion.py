...
class IntelligentRRFFusion:
    ...
    def __init__(self, redis_client=None, intent_processor=None):
        ...
        self.genetic_boost_enabled = os.getenv("RRF_GENETIC_BOOST", "true").lower() == "true"
        self.debug_mode = os.getenv("RRF_DEBUG_MODE", "false").lower() == "true"
        
        logger.info(f"IntelligentRRF initialisé - Enabled: {self.enabled}, Learning: {self.learning_mode}")
    
    async def enhanced_fusion(self, vector_results: List[Dict], bm25_results: List[Dict],
                             alpha: float, top_k: int, query_context: Dict,
                             intent_result=None) -> List[Dict]:
        """Point d'entrée principal pour la fusion RRF intelligente"""
        
        if not self.enabled:
            return await self._classic_rrf_fallback(vector_results, bm25_results, alpha, top_k)
        ...
        try:
            ...
            if self.learning_mode:
                await self._record_fusion_performance(context, final_results, time.time() - start_time)
            
            if self.debug_mode:
                logger.debug(f"RRF intelligent: {len(final_results)} docs, "
    ...
    async def _analyze_query_context(self, query_context: Dict, intent_result) -> Dict:
        ...
        genetic_line = entities.get('line', '') or self._detect_genetic_line(query_lower)
        ...
        context = {
            ...
            "is_comparative": any(word in query_lower for word in ['vs', 'versus', 'contre', 'par rapport']),
    ...
    async def _multi_method_fusion(self, vector_results: List[Dict], bm25_results: List[Dict],
                                  params: AdaptiveRRFParams, context: Dict) -> List[Dict]:
    ...
    async def _weighted_rrf_fusion(self, vector_results: List[Dict], bm25_results: List[Dict],
                                  params: AdaptiveRRFParams, context: Dict) -> List[Dict]:
        ...
        for content_key, data in all_docs.items():
            ...
            if data["vector_rank"]:
                vector_contribution = (params.vector_weight * data["specialization_boost"]) / \
                                    (params.rrf_k + data["vector_rank"])
                rrf_score += vector_contribution
            
            # Composante BM25
            if data["bm25_rank"]:
                bm25_contribution = params.bm25_weight / (params.rrf_k + data["bm25_rank"])
    ...
    async def _poultry_specialized_ranking(self, candidates: List[Dict], 
                                         context: Dict, top_k: int) -> List[Dict]:
        ...
        if context.get("genetic_line") and context.get("has_performance_metric"):
            diverse_results = self._boost_genetic_metric_coherence(diverse_results, context)
    ...
    def _calculate_poultry_boost(self, doc: Dict, context: Dict) -> float:
        ...
        if (context.get("genetic_line") and context.get("has_performance_metric") and
            genetic_line in content and any(metric in content for metric in context.get("performance_metrics", []))):
            boost *= 1.5
        
        # Boost source technique
        source_type = doc.get("metadata", {}).get("source", "")
        if any(keyword in source_type.lower() for keyword in ["guide", "référence", "technique", "standard"]):
            boost *= 1.2
    ...
    def _classify_query_type(self, query_lower: str, entities: Dict) -> QueryType:
        ...
        if (any(metric in query_lower for metric in ["fcr", "poids", "mortalité", "ponte"]) and
            any(genetic in query_lower for genetic in ["ross", "cobb", "hubbard", "isa"])):
            return QueryType.METRIC_EXACT
        
        # Requête comparative
        if any(comp in query_lower for comp in ["vs", "versus", "contre", "par rapport", "différence", "compare"]):
            return QueryType.COMPARISON
        
        # Requête conceptuelle
        if any(concept in query_lower for concept in ["comment", "pourquoi", "expliquer", "améliorer", "optimiser"]):
            return QueryType.CONCEPTUAL
        
        # Requête diagnostic
        if any(diag in query_lower for diag in ["problème", "symptôme", "maladie", "diagnostic", "traitement"]):
    ...
    async def _classic_rrf_fallback(self, vector_results: List[Dict], bm25_results: List[Dict],
    ...
    async def _record_fusion_performance(self, context: Dict, results: List[Dict], processing_time: float):
        ...
        try:
            ...
            await self.redis.setex(redis_key, 86400, json.dumps(asdict(metric)))  # 24h TTL
    ...
    def _apply_diversity_filter(self, candidates: List[Dict], context: Dict) -> List[Dict]:
    ...
    def _boost_medical_urgency(self, results: List[Dict]) -> List[Dict]:
        ...
        medical_keywords = ["traitement", "diagnostic", "symptôme", "maladie", "vétérinaire"]
    ...
    def _boost_genetic_metric_coherence(self, results: List[Dict], context: Dict) -> List[Dict]:
    ...
    async def _score_interpolation_fusion(self, vector_results: List[Dict], bm25_results: List[Dict],
                                        params: AdaptiveRRFParams, context: Dict) -> List[Dict]:
        """Fusion par interpolation de scores"""
        # Implémentation alternative simple
        return await self._weighted_rrf_fusion(vector_results, bm25_results, params, context)
    
    async def _rbp_fusion(self, vector_results: List[Dict], bm25_results: List[Dict],
                         params: AdaptiveRRFParams, context: Dict) -> List[Dict]:
