...
class ValidationCore:
    ...
    # ========================================================================
    # CONFIGURATION CENTRALISÉE - BREEDS
    # ========================================================================
    ...
    KNOWN_BREEDS = {
        "cobb500": {
            "name": "Cobb 500",
            "age_range": (1, 57),
            "aliases": ["cobb 500", "cobb-500", "500"],
            "genetic_line": "Cobb",
        },
        "ross308": {
            "name": "Ross 308",
            "age_range": (0, 56),
            "aliases": ["ross 308", "ross-308", "308", "308 ff"],
            "genetic_line": "Ross",
        },
        "hubbard": {
            "name": "Hubbard",
            "age_range": (1, 56),
            "aliases": ["hubbard classic", "hubbard flex"],
            "genetic_line": "Hubbard",
        },
        "arbor_acres": {
            "name": "Arbor Acres",
            "age_range": (1, 56),
            "aliases": ["arbor acres", "aa"],
            "genetic_line": "Arbor Acres",
        },
    }
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        logger.info(f"ValidationCore initialisé (strict_mode={strict_mode})")
    ...
    def _validate_breed(self, breed: str) -> Dict[str, Any]:
        ...
        """
        breed_normalized = breed.lower().replace(" ", "").replace("-", "")
        issues = []
        alternatives = []

        # Recherche exacte
        if breed_normalized in self.KNOWN_BREEDS:
            return {"issues": [], "alternatives": []}

        # Recherche par alias
        for breed_key, breed_info in self.KNOWN_BREEDS.items():
            for alias in breed_info["aliases"]:
                alias_normalized = alias.lower().replace(" ", "").replace("-", "")
                if breed_normalized == alias_normalized:
                    issues.append(
                        ValidationIssue(
                            field="breed",
                            severity=ValidationSeverity.INFO,
                            message=f"Souche '{breed}' reconnue comme {breed_info['name']}",
                        )
                    )
                    return {"issues": issues, "alternatives": []}

        # Recherche partielle (similitude)
        for breed_key, breed_info in self.KNOWN_BREEDS.items():
            if breed_normalized in breed_key or breed_key in breed_normalized:
                alternatives.append(breed_key)

        # Si alternatives trouvées
        if alternatives:
            issues.append(
                ValidationIssue(
                    field="breed",
                    ...
                    suggestion=f"Souches similaires: {', '.join(alternatives)}",
                )
            )
        else:
            # Aucune correspondance
            available_breeds = [info["name"] for info in self.KNOWN_BREEDS.values()]
            issues.append(
                ValidationIssue(
                    field="breed",
                    severity=ValidationSeverity.ERROR,
            ...
            alternatives = list(self.KNOWN_BREEDS.keys())

        return {"issues": issues, "alternatives": alternatives}

    def _validate_age(self, age: int, breed: Optional[str] = None) -> Dict[str, Any]:
        """
        Valide l'âge avec contexte de souche
        ...
        # Validation avec contexte breed
        if breed:
            breed_normalized = breed.lower().replace(" ", "")
            if breed_normalized in self.KNOWN_BREEDS:
                min_age, max_age = self.KNOWN_BREEDS[breed_normalized]["age_range"]
                ...
                if age < min_age:
                    issues.append(
                        ValidationIssue(
                            field="age_days",
                            severity=ValidationSeverity.WARNING,
                            message=f"Âge {age}j inférieur à la plage typique pour {breed} ({min_age}-{max_age}j)",
                            suggestion=f"Les données pour {breed} commencent généralement à {min_age}j",
                        )
                    )
                    ...
                    issues.append(
                        ValidationIssue(
                            field="age_days",
                            severity=ValidationSeverity.WARNING,
                            message=f"Âge {age}j supérieur à la plage typique pour {breed} ({min_age}-{max_age}j)",
                            suggestion=f"Les données pour {breed} vont généralement jusqu'à {max_age}j",
                        )
                    )
    ...
    def validate_query_feasibility(
        ...
        if self._is_nutrition_query(query_lower) and entities.get(
            "breed", ""
        ).lower().startswith("ross"):
            result.add_issue(
                field="query_type",
                severity=ValidationSeverity.WARNING,
                message="Données nutritionnelles limitées pour Ross 308",
