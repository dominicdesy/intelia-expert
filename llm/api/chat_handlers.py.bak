...
class ChatHandlers:
    ...
    async def handle_clarification_context(
        ...
        if self.context_manager.is_clarification_response(message, pending_context):
            ...
            # ÉTAPE 4: Accumulation normale (fusion intelligente)
            ...
            # Retourner la requête accumulée pour traitement
            return {
                "accumulated_query": pending_context["original_query"],
                "language": language,
                "continue_processing": True,
                "merged_entities": pending_context.get("partial_entities", {}),
            }
