...
from ..config.config import (
    MAX_CONVERSATION_CONTEXT, TENANT_TTL, MAX_TENANTS, STREAM_CHUNK_LEN, ENABLE_METRICS_LOGGING
)
from ..utils.utilities import (
    safe_serialize_for_json, safe_get_attribute, safe_dict_get,
    sse_event, smart_chunk_text, get_out_of_domain_message, 
    get_aviculture_response, MetricsCollector, detect_language_enhanced
)
from ..utils.monitoring import SystemHealthMonitor
from ..core.memory import ConversationMemory
# ✅ CORRECTION: Import depuis utils au lieu de core
from ..utils.imports_and_dependencies import quick_connectivity_check, get_full_status_report
        ...
        try:
            ...
            if metadata and isinstance(metadata, dict):
                try:
                    ...
                    if isinstance(opt_stats, dict):
                        self.endpoint_metrics["cache_hits"] += int(safe_dict_get(opt_stats, "cache_hits", 0))
                        self.endpoint_metrics["cache_misses"] += int(safe_dict_get(opt_stats, "cache_misses", 0))
                        self.endpoint_metrics["semantic_cache_hits"] += int(safe_dict_get(opt_stats, "semantic_cache_hits", 0))
                        self.endpoint_metrics["hybrid_searches"] += int(safe_dict_get(opt_stats, "hybrid_searches", 0))
                        self.endpoint_metrics["genetic_boosts_applied"] += int(safe_dict_get(opt_stats, "genetic_boosts_applied", 0))
                        self.endpoint_metrics["rrf_learning_updates"] += int(safe_dict_get(opt_stats, "rrf_learning_updates", 0))
            ...
            if processing_time > 0:
                ...
                if self.recent_processing_times:
                    self.endpoint_metrics["avg_processing_time"] = (
            ...
            if confidence > 0:
                ...
                if self.recent_confidences:
                    self.endpoint_metrics["avg_confidence"] = (
    ...
    def get_metrics(self) -> Dict:
        ...
        try:
            ...
            return {
                **self.endpoint_metrics,
                "success_rate": (
                    (self.endpoint_metrics["rag_enhanced_queries"] + self.endpoint_metrics["verified_responses"] + self.endpoint_metrics["agent_queries"]) / total_queries
                ),
