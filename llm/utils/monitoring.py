...
from ..config.config import (
    OPENAI_API_KEY, LANGSMITH_ENABLED, LANGSMITH_API_KEY, LANGSMITH_PROJECT,
    ENABLE_INTELLIGENT_RRF, RRF_LEARNING_MODE, RRF_GENETIC_BOOST,
    get_config_status, validate_config
)
from .imports_and_dependencies import (
    require_critical_dependencies, get_full_status_report, 
    quick_connectivity_check, OPENAI_AVAILABLE, WEAVIATE_AVAILABLE
)
            ...
            if rag_engine and safe_get_attribute(rag_engine, 'is_initialized', False):
                try:
                    ...
                    global_status["services"]["rag_engine"] = {
                        "status": "healthy" if not getattr(rag_engine, 'degraded_mode', False) else "degraded",
