...
class DependencyManager:
    ...
    def _load_internal_modules(self):
        ...
        internal_modules = {
            ...
            'embedder': {
                'path': 'retrieval.embedder',
                'class': 'OpenAIEmbedder'
            },
            'rag_engine': {
                'path': 'core.rag_engine',
                'class': 'InteliaRAGEngine'
            },
            'cache_core': {
                'path': 'cache.cache_core',
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(module, class_name):
                        ...
                        logger.debug(f"Module {module_name} importé avec succès depuis {module_path}")
                    else:
                        raise ImportError(f"Classe {class_name} non trouvée dans {module_path}")
                        
                except ImportError as e:
                    logger.warning(f"Erreur import {module_name} depuis {module_path}: {e}")
                    self.dependencies[module_name] = DependencyInfo(
                        name=module_name,
                        status=DependencyStatus.MISSING,
                        error_message=str(e),
...
async def quick_connectivity_check(redis_client=None, weaviate_client=None) -> Dict[str, bool]:
    ...
    openai_ok = globals().get('OPENAI_AVAILABLE', False)
...
def validate_imports_corrections() -> Dict[str, bool]:
    ...
    validation_results = {
        ...
        "wvc_query_imported": globals().get('wvc_query') is not None,
        "openai_clients": (
