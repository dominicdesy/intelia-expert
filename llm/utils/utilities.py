# -*- coding: utf-8 -*-
"""
utilities.py - Fonctions utilitaires unifiÃ©es COMPLÃˆTES
Version modulaire avec toutes les fonctions nÃ©cessaires
"""
...
class ProcessingResult:
    """RÃ©sultat de traitement d'une requÃªte"""
    success: bool
...
# ============================================================================
# COLLECTEUR DE MÃ‰TRIQUES (Version complÃ¨te)
# ============================================================================

class MetricsCollector:
    """Collecteur de mÃ©triques enrichi avec statistiques intent et cache sÃ©mantique"""
    def __init__(self):
    ...
    def as_json(self) -> dict:
        """Export JSON des mÃ©triques pour l'app"""
        return {
...
def get_aviculture_response(message: str, language: str = "fr") -> str:
    """GÃ©nÃ¨re une rÃ©ponse aviculture basique si le RAG Ã©choue"""
    
    message_lower = message.lower()
    
    # RÃ©ponses par sujet aviculture
    if any(term in message_lower for term in ['fcr', 'conversion', 'indice']):
        if language == "fr":
            return """L'indice de conversion alimentaire (FCR) optimal varie selon l'Ã¢ge et la souche :
...
- **Facteurs influenÃ§ant le FCR** :
  - QualitÃ© de l'aliment et formulation
  - TempÃ©rature et ventilation du bÃ¢timent
  - DensitÃ© d'Ã©levage
  - SantÃ© du troupeau
...
- **Facteurs de croissance** :
  - Nutrition adaptÃ©e par phase
  - TempÃ©rature optimale (32Â°C dÃ©marrage, baisse progressive)
  - DensitÃ© max 30-33 kg/mÂ²
  - Ã‰clairage progressif
  - Prophylaxie sanitaire

Surveillez l'uniformitÃ© du lot (CV < 10%) et ajustez l'alimentation si Ã©cart aux standards."""
        else:
    ...
    elif any(term in message_lower for term in ['tempÃ©rature', 'temperature', 'ventilation', 'climat']):
        if language == "fr":
            return """**Programme tempÃ©rature poulets de chair :**

- **DÃ©marrage (0-7 jours)** : 32-34Â°C
- **Croissance (8-21 jours)** : RÃ©duction 2-3Â°C/semaine
- **Finition (22-42 jours)** : 18-22Â°C

- **Ventilation** :
  - Minimum : 0.5 mÂ³/h/kg vif
  - Maximum : 3-4 mÂ³/h/kg vif  
  - Vitesse air max : 2.5 m/s

- **HygromÃ©trie** : 60-70%

**Points clÃ©s** :
- Ã‰viter les chocs thermiques
- Ventilation progressive selon Ã¢ge
- Surveillance des zones froides/chaudes
- Ajustement selon saison"""
        else:
            return "Broiler temperature program: Start 32-34Â°C (0-7 days), reduce 2-3Â°C/week, finish 18-22Â°C (22-42 days). Ventilation: 0.5-4 mÂ³/h/kg, max air speed 2.5 m/s."
    
    elif any(term in message_lower for term in ['mortalitÃ©', 'mortality', 'santÃ©', 'health']):
        if language == "fr":
            return """**Objectifs mortalitÃ© poulets de chair :**
...
**PrÃ©vention :**
- Programme de dÃ©marrage rigoureux
- Vaccination adaptÃ©e
- BiosÃ©curitÃ© stricte
- Surveillance quotidienne
- NÃ©cropsies systÃ©matiques"""
        else:
...
**STARTER (0-10 jours) :**
- ProtÃ©ines : 22-23%
- Ã‰nergie : 3000-3050 kcal/kg
- PrÃ©sentation : miettes 2-3mm

**CROISSANCE (11-25 jours) :**
- ProtÃ©ines : 20-21%
- Ã‰nergie : 3100-3150 kcal/kg  
- PrÃ©sentation : granulÃ©s 3-4mm

**FINITION (26-42 jours) :**
- ProtÃ©ines : 18-19%
- Ã‰nergie : 3150-3200 kcal/kg
- PrÃ©sentation : granulÃ©s 4-5mm
...
- Transition progressive entre phases (2-3 jours)"""
        ...
        # RÃ©ponse gÃ©nÃ©rale aviculture
        if language == "fr":
            return """Je suis spÃ©cialisÃ© dans l'aviculture et l'Ã©levage de poulets de chair. Je peux vous aider sur :

- **Performances** : FCR, poids, croissance, mortalitÃ©
- **Nutrition** : Programmes alimentaires, formulation
- **Environnement** : TempÃ©rature, ventilation, densitÃ©
- **SantÃ©** : PrÃ©vention, vaccination, biosÃ©curitÃ©
- **Technique** : Ã‰quipements, bÃ¢timents, gestion

Posez-moi une question prÃ©cise sur l'un de ces domaines !"""
        else:
...
def get_all_metrics_json(METRICS: MetricsCollector, extra: dict | None = None) -> dict:
    """Fonction d'export JSON consolidÃ©e des mÃ©triques avec donnÃ©es supplÃ©mentaires"""
...
def detect_language_enhanced(text: str, default: str = "fr") -> str:
    """DÃ©tection de langue optimisÃ©e pour requÃªtes courtes et techniques"""
...
# ============================================================================
# DONNÃ‰ES DE TEST ET VALIDATION
# ============================================================================

COMPREHENSIVE_TEST_QUERIES = [
    "Quel est le poids cible Ã  21 jours pour du Ross 308?",
    "FCR optimal pour poulet de chair Cobb 500 Ã  35 jours",
    "Consommation d'eau Ã  28 jours pour Ã©levage tunnel",
    "TempÃ©rature de dÃ©marrage pour poussins en tunnel",
    "Ventilation minimale Ã  14 jours Ross 308",
    "HumiditÃ© optimale phase starter",
    "Programme de vaccination pour reproducteur",
    "Protocole biosÃ©curitÃ© couvoir",
    "DensitÃ© optimale en Ã©levage au sol",
    "Mes poulets ont des signes respiratoires",
    "MortalitÃ© Ã©levÃ©e Ã  10 jours que faire",
    "SymptÃ´mes Newcastle chez reproducteurs",
    "CoÃ»t alimentaire par kg de poids vif produit",
    "Performance EPEF Ross 308 standard",
    "Marge bÃ©nÃ©ficiaire par sujet abattu",
    "Ross-308 35j FCR",
    "C-500 poids 42 jours",
    "Hubbard Flex vaccination",
    "ISA Brown ponte pic",
    "R308 dÃ©marrage tempÃ©rature",
    "MÃ©tÃ©o demain",  # Hors domaine
    "Comment Ã©lever des chats",  # Hors domaine
    "Performance globale exploitation complÃ¨te multi-bÃ¢timents",  # Complexe
]
...
class IntentProcessorFactory:
    """Factory robuste pour crÃ©er des processeurs d'intentions"""
    
    @staticmethod
    def create_processor(intents_file_path: Optional[str] = None, 
                        validate_on_creation: bool = True) -> 'IntentProcessor':
        try:
            from processing.intent_processor import IntentProcessor, ConfigurationError
        ...
        if intents_file_path is None:
            ...
            logger.info(f"Utilisation du chemin par dÃ©faut: {intents_file_path}")
        ...
        try:
            ...
            if health.get('status') == 'critical':
                logger.error(f"Processeur crÃ©Ã© mais en Ã©tat critique: {health.get('reason')}")
                raise RuntimeError(f"Processeur en Ã©tat critique: {health.get('reason')}")
            
            logger.info(f"IntentProcessor crÃ©Ã© avec succÃ¨s - Statut: {health.get('status', 'unknown')}")
            return processor
            
        except Exception as e:
            logger.error(f"Erreur crÃ©ation IntentProcessor: {e}")
            raise RuntimeError(f"Impossible de crÃ©er IntentProcessor: {e}")
...
def process_query_with_intents(processor, query: str, 
    ...
    if not query or not query.strip():
        return ProcessingResult(
            success=False,
            error_message="RequÃªte vide ou invalide",
            processing_time=0.0
        )
    ...
    try:
        ...
        if not result:
            return ProcessingResult(
                success=False,
                error_message="Aucun rÃ©sultat retournÃ© par le processeur",
        ...
        logger.error(f"Erreur traitement requÃªte '{query[:50]}...': {e}")
        return ProcessingResult(
            success=False,
    ...
    try:
        ...
        if not config_file.exists():
            return ValidationReport(
                is_valid=False,
                errors=[f"Fichier non trouvÃ©: {config_path}"],
                warnings=[],
                stats={},
                recommendations=["VÃ©rifiez le chemin du fichier de configuration"]
            )
        ...
        try:
            ...
            return ValidationReport(
                ...
                warnings=[],
                stats={},
                recommendations=["VÃ©rifiez la syntaxe JSON avec un validateur"]
            )
        ...
        for section in required_sections:
            if section not in config:
                ...
                errors.append(f"Section {section} doit Ãªtre un dictionnaire")
...
# Fonction principale de crÃ©ation (pour compatibilitÃ©)
def create_intent_processor(intents_file_path: Optional[str] = None) -> 'IntentProcessor':
    """Factory principale pour crÃ©er un processeur d'intentions"""
