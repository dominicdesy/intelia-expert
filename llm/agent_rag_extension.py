...
# Imports de votre système existant
from rag_engine import InteliaRAGEngine, RAGResult, RAGSource
...
class SubQuery:
    ...
    dependencies: List[str] = field(default_factory=list)  # IDs de sous-requêtes prérequises
...
class QueryDecomposer:
    ...
    def analyze_complexity(self, query: str, intent_result: IntentResult) -> QueryComplexity:
        ...
        words = query.split()
        if len(words) > 20 and any(word in query_lower for word in ['et', 'and', 'aussi', 'également']):
            return QueryComplexity.MULTI_METRIC
        
        return QueryComplexity.SIMPLE
    
    async def decompose_query(self, query: str, intent_result: IntentResult, complexity: QueryComplexity) -> List[SubQuery]:
    ...
    async def _decompose_multi_metric(self, query: str, entities: Dict) -> List[SubQuery]:
        ...
        base_context = f"lignée {entities.get('line', '')}" if 'line' in entities else ""
        age_context = f"âge {entities.get('age_days', entities.get('age_weeks', ''))} jours" if 'age_days' in entities or 'age_weeks' in entities else ""
    ...
    async def _decompose_comparative(self, query: str, entities: Dict) -> List[SubQuery]:
    ...
    async def _decompose_conditional(self, query: str, entities: Dict) -> List[SubQuery]:
        ...
        if 'si' in query.lower():
            ...
            if len(parts) > 1:
                ...
                action = parts[1].split('alors')[1].strip() if 'alors' in parts[1] else "que faire?"
        ...
        return [SubQuery(query=query, intent_type=IntentType.GENERAL_POULTRY, priority=1)]
    ...
    async def _decompose_diagnostic(self, query: str, entities: Dict) -> List[SubQuery]:
        ...
        # 2. Causes possibles
        sub_queries.append(SubQuery(
            query=f"Causes possibles des symptômes décrits",
            intent_type=IntentType.DIAGNOSIS_TRIAGE,
            priority=2,
        ...
        sub_queries.append(SubQuery(
            query=f"Protocole d'action pour ces symptômes",
            intent_type=IntentType.PROTOCOL_QUERY,
            priority=3,
...
class MultiDocumentSynthesizer:
    ...
    async def synthesize_results(self, original_query: str, sub_results: List[RAGResult], complexity: QueryComplexity) -> str:
    ...
    async def _synthesize_multi_metric(self, query: str, results: List[RAGResult]) -> str:
...
RÉPONSE SYNTHÉTISÉE:"""
    ...
    async def _synthesize_comparative(self, query: str, results: List[RAGResult]) -> str:
...
class InteliaAgentRAG(InteliaRAGEngine):
    ...
    async def process_query_agent(self, query: str, language: str = "fr", tenant_id: str = "") -> AgentResult:
        ...
        try:
            ...
            if complexity == QueryComplexity.SIMPLE:
                ...
                sub_queries = await self.decomposer.decompose_query(query, intent_result, complexity)
                agent_decisions.append(f"Décomposé en {len(sub_queries)} sous-requêtes")
                self.agent_stats["avg_sub_queries"] = self._update_avg_sub_queries(len(sub_queries))
                ...
                agent_decisions.append(f"{len(valid_sub_results)}/{len(sub_queries)} sous-requêtes réussies")
                ...
                if valid_sub_results:
                    ...
                    avg_confidence = sum(r.confidence for r in valid_sub_results) / len(valid_sub_results)
                    synthesis_bonus = 0.1 if len(valid_sub_results) > 1 else 0
                    final_confidence = min(0.95, avg_confidence + synthesis_bonus)
                    
                    self.agent_stats["synthesis_success_rate"] = self._update_success_rate(True)
                    ...
                    self.agent_stats["synthesis_success_rate"] = self._update_success_rate(False)
            ...
            try:
                rag_result = await super().process_query(query, language, tenant_id)
                return AgentResult(
                    final_answer=rag_result.answer or f"Erreur agent, fallback utilisé: {str(e)}",
                    ...
                    decomposition_used=False,
                    agent_decisions=[f"Erreur agent: {str(e)}", "Fallback vers RAG standard"]
                )
    ...
    async def _process_sub_query(self, sub_query: SubQuery, language: str, tenant_id: str) -> RAGResult:
...
async def process_query_with_agent(agent_engine: InteliaAgentRAG, query: str, language: str = "fr", tenant_id: str = "") -> AgentResult:
