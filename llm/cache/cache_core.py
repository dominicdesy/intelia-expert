...
class CacheConfig:
    ...
    def from_env(cls) -> 'CacheConfig':
        ...
        return cls(
            ...
            max_value_bytes=int(os.getenv("CACHE_MAX_VALUE_BYTES", cls.max_value_bytes)),
            max_keys_per_namespace=int(os.getenv("CACHE_MAX_KEYS_PER_NS", cls.max_keys_per_namespace)),
            total_memory_limit_mb=int(os.getenv("CACHE_TOTAL_MEMORY_LIMIT_MB", cls.total_memory_limit_mb)),
            warning_threshold_mb=int(os.getenv("CACHE_WARNING_THRESHOLD_MB", cls.warning_threshold_mb)),
            purge_threshold_mb=int(os.getenv("CACHE_PURGE_THRESHOLD_MB", cls.purge_threshold_mb)),
            enable_auto_purge=os.getenv("CACHE_ENABLE_AUTO_PURGE", "true").lower() == "true",
            enable_compression=os.getenv("CACHE_ENABLE_COMPRESSION", "false").lower() == "true",
            stats_log_interval=int(os.getenv("CACHE_STATS_LOG_INTERVAL", cls.stats_log_interval)),
        )
...
class RedisCacheCore:
    ...
    def _get_config_summary(self) -> str:
        ...
        return (f"limite={self.config.total_memory_limit_mb}MB, "
    ...
    async def get(self, key: str, namespace: str = 'default') -> Optional[Any]:
        ...
        try:
            ...
            value = self._decompress_value(raw_value) if self.config.enable_compression else raw_value
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None, 
        ...
        try:
            ...
            if len(serialized) > self.config.max_value_bytes:
                logger.warning(f"Valeur trop grande ignorée: {len(serialized)} bytes > {self.config.max_value_bytes}")
                self.protection_stats["oversized_values"] += 1
                return False
            
            # Compression si activée
            final_value = self._compress_value(serialized) if self.config.enable_compression else serialized
            ...
            success = await asyncio.wait_for(
                self.client.set(full_key, final_value, ex=effective_ttl),
                timeout=1.0
            )
    ...
    async def _check_size_and_namespace_quota(self, namespace: str, data: bytes) -> bool:
    ...
    def _handle_cache_error(self, error_msg: str):
        ...
        if self.consecutive_errors >= self.max_consecutive_errors:
            backoff_seconds = min(300, 2 ** (self.consecutive_errors - self.max_consecutive_errors))
            self.error_backoff_until = time.time() + backoff_seconds
            self.status = CacheStatus.CRITICAL
            
            logger.error(f"Cache en backoff pour {backoff_seconds}s après {self.consecutive_errors} erreurs")
    ...
    async def _check_memory_limits(self) -> bool:
        ...
        try:
            ...
            usage_percent = (self.stats.memory_usage_mb / self.config.total_memory_limit_mb) * 100
            
            if usage_percent > (self.config.purge_threshold_mb / self.config.total_memory_limit_mb) * 100:
                if self.config.enable_auto_purge:
                    await self._auto_purge_cache()
                return False
            elif usage_percent > (self.config.warning_threshold_mb / self.config.total_memory_limit_mb) * 100:
    ...
    async def _update_memory_stats(self):
        ...
        try:
            ...
            if total_keys > 0:
                self.stats.avg_value_size = (used_memory_bytes / total_keys) if total_keys > 0 else 0
    ...
    async def _auto_purge_cache(self):
        ...
        try:
            ...
            namespaces_by_priority = ['searches', 'responses', 'intents', 'normalized', 'embeddings']
            
            for namespace in namespaces_by_priority:
                keys_deleted = await self._purge_namespace(namespace, self.config.purge_ratio)
                if keys_deleted > 0:
    ...
    async def _monitor_namespace_limits(self, namespace: str):
        ...
        try:
            ...
            if len(keys) > self.config.max_keys_per_namespace:
                logger.warning(f"Namespace {namespace} dépasse la limite: {len(keys)} clés")
        ...
        for namespace, prefix in self.namespaces.items():
            try:
                ...
                stats[namespace] = {
                    ...
                    "usage_percent": (len(keys) / self.config.max_keys_per_namespace) * 100,
