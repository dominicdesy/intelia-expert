...
def benchmark_regex_compilation():
    ...
    for _ in range(NUM_ITERATIONS):
        ...
        for pattern, replacement, flags in patterns_config:
            response = re.sub(
                pattern, replacement, response, flags=flags if flags else 0
            )
    ...
    print("\nResults:")
    print(
        f"   Baseline (compile each time):  {baseline_avg:.3f}ms +/- {statistics.stdev(baseline_times):.3f}ms"
    )
    print(
        f"   Optimized (pre-compiled):      {optimized_avg:.3f}ms +/- {statistics.stdev(optimized_times):.3f}ms"
    )
    print(
        f"   [OK] Improvement:              {improvement:.3f}ms ({improvement_pct:.1f}% faster)"
    )
...
def benchmark_veterinary_keyword_lookup():
    ...
    for _ in range(NUM_ITERATIONS):
        ...
        _ = bool(veterinary_keywords_set & query_words)  # found flag
    ...
    print("\nResults:")
    print(
        f"   Baseline (linear search):      {baseline_avg:.3f}ms +/- {statistics.stdev(baseline_times):.3f}ms"
    )
    print(
        f"   Optimized (set intersection):  {optimized_avg:.3f}ms +/- {statistics.stdev(optimized_times):.3f}ms"
    )
    print(
        f"   [OK] Improvement:              {improvement:.3f}ms ({improvement_pct:.1f}% faster)"
    )
...
def benchmark_postprocessor_caching():
    ...
    print("\nResults:")
    print(
        f"   Baseline (create each time):   {baseline_avg:.3f}ms +/- {statistics.stdev(baseline_times):.3f}ms"
    )
    print(
        f"   Optimized (cached):            {optimized_avg:.3f}ms +/- {statistics.stdev(optimized_times):.3f}ms"
    )
    print(
        f"   [OK] Improvement:              {improvement:.3f}ms ({improvement_pct:.1f}% faster)"
    )
...
def main():
    ...
    print("\nExpected vs Actual:")
    print("   Expected improvement:          ~10ms")
