...
# Configuration mémoire tenant - NOUVEAU
MAX_TENANTS = int(os.getenv("MAX_TENANTS", "200"))   # borne dure
TENANT_TTL = int(os.getenv("TENANT_TTL_SEC", "86400"))  # 24h par défaut
...
# -----------------------------------------------------------------------------
# CORRECTION 1: Mémoire de conversation bornée par tenant avec LRU + TTL
# -----------------------------------------------------------------------------
class TenantMemory(OrderedDict):
    ...
    def set(self, tenant_id: str, item: list):
        """Ajoute/met à jour un tenant avec gestion LRU et TTL"""
        now = time.time()
        self[tenant_id] = {"data": item, "ts": now}
        self.move_to_end(tenant_id)  # LRU: déplacer vers la fin
        
        # Purge TTL (inactifs depuis trop longtemps)
        expired_keys = [k for k, v in self.items() if now - v["ts"] > TENANT_TTL]
        for k in expired_keys:
            del self[k]
            logger.debug(f"Tenant {k} purgé (TTL expiré)")
    ...
    def get(self, tenant_id: str, default=None):
        """Récupère un tenant et le marque comme récemment utilisé"""
        ...
        if now - self[tenant_id]["ts"] > TENANT_TTL:
            del self[tenant_id]
            logger.debug(f"Tenant {tenant_id} purgé à l'accès (TTL expiré)")
            return default
    ...
    def get_stats(self) -> Dict:
        """Statistiques de la mémoire tenant"""
        now = time.time()
...
def add_to_conversation_memory(tenant_id: str, question: str, answer: str):
    """Version corrigée avec TenantMemory"""
    ...
    # Limiter le nombre d'échanges par tenant
...
def build_context_prompt(tenant_id: str, current_question: str) -> str:
    """Inchangé, mais utilise la nouvelle TenantMemory"""
    ...
    for item in history[-2:]:  # 2 derniers échanges pour contexte
...
async def startup_event():
    ...
    logger.info("🚀 Démarrage de l'application Intelia Expert")
    await initialize_rag_engine()

async def shutdown_event():
    """NOUVEAU: Événement d'arrêt pour nettoyer les ressources"""
    global rag_engine
    try:
        ...
        logger.error(f"❌ Erreur lors du nettoyage d'arrêt: {e}")
...
def health():
    ...
    # Stats mémoire de conversation - CORRIGÉ avec la nouvelle TenantMemory
    ...
    return {
        ...
        "conversation_memory": memory_stats,  # NOUVEAU: stats détaillées
...
async def debug_nli_detailed():
    """Diagnostic détaillé du classificateur NLI - VERSION CORRIGÉE MULTIPLATEFORME"""
    import traceback
    ...
    try:
        ...
        async def load_model_with_timeout():
            """Charge le modèle de façon asynchrone"""
            def _load_model():
                return pipeline(
        ...
        try:
            ...
            # Test aussi dans un thread pour éviter de bloquer
            loop = asyncio.get_event_loop()
...
# NOUVEAU: Route pour diagnostics mémoire
@router.get("/debug/memory")
def debug_memory():
    """Diagnostic détaillé de la mémoire tenant"""
    global conversation_memory
    
    stats = conversation_memory.get_stats()
    
    # Analyse par tenant (sans exposer les données sensibles)
    tenants_analysis = []
    now = time.time()
    ...
    for tenant_id, tenant_data in list(conversation_memory.items()):
        ...
        tenants_analysis.append({
            "tenant_id": tenant_id[:8] + "..." if len(tenant_id) > 8 else tenant_id,  # Masquer pour la vie privée
    ...
    return {
        **stats,
        "tenant_details": tenants_analysis[:20],  # Limite pour éviter la surcharge
        "config": {
            "max_tenants": MAX_TENANTS,
            "tenant_ttl_sec": TENANT_TTL,
            "max_memory_items": MAX_MEMORY_ITEMS
...
async def chat_stream(request: Request):
    ...
    Version avec intégration RAG complète et mémoire optimisée
    """
    try:
        payload = await request.json()
    except Exception as e:
