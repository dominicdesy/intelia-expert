...
class QueryDecomposer:
    ...
    def detect_complexity(self, query: str) -> bool:
        ...
        factors_found = sum(
            1 for pattern in self.factor_patterns.values()
            if re.search(pattern, query_lower)
        )
    ...
    def extract_multi_factors_explicit(self, query: str) -> List[str]:
        ...
        impact_match = re.search(
            r'(impact|effet|influence)\s+(?:de\s+|d\')?(.+?)\s+sur',
            query_lower
        )
    ...
    def decompose(self, query: str) -> DecompositionResult:
        ...
        logger.info(f"Decomposed into {len(sub_queries)} sub-queries, strategy: {aggregation}")
    ...
    def execute_subqueries(
        ...
        for idx, sub_query in enumerate(sub_queries):
            try:
                logger.debug(f"Executing sub-query {idx+1}/{len(sub_queries)}: '{sub_query.query}'")
        ...
        logger.info(f"Executed {len(results)} sub-queries ({len([r for r in results if 'error' not in r])} successful)")
    ...
    def _aggregate_compare(
        ...
        for i in range(len(results)):
            for j in range(i + 1, len(results)):
                factor_a = results[i].get('sub_query_context', {}).get('factor', f'Factor {i}')
                factor_b = results[j].get('sub_query_context', {}).get('factor', f'Factor {j}')

                comparison['comparisons'].append({
                    'factor_a': factor_a,
                    'factor_b': factor_b,
                    'result_a': results[i].get('answer', results[i].get('content', 'N/A')),
                    'result_b': results[j].get('answer', results[j].get('content', 'N/A'))
                })
    ...
    def _aggregate_synthesize(
        ...
        for result in results:
            ...
            synthesis['synthesis_parts'].append({
                'factor': factor,
                'insight': answer
            })
