-- ============================================================================
-- Ajouter un abonnement Essential pour dominic.desy@icloud.com (VERSION 3)
-- Date: 2025-01-18
-- Description: Version simplifiée basée sur la structure réelle de user_billing_info
-- ============================================================================

-- ÉTAPE 0: Créer l'entrée user_billing_info (OBLIGATOIRE)
-- Structure réelle: user_email (NOT NULL), tous les autres ont des defaults
...
SELECT
    'SUCCÈS - Vérification des données' as status,
    '' as detail
UNION ALL
SELECT '---', '---'
UNION ALL
SELECT '1. USER_BILLING_INFO',
       CASE WHEN COUNT(*) > 0 THEN '✅ Créé' ELSE '❌ Manquant' END
FROM user_billing_info
WHERE user_email = 'dominic.desy@icloud.com'
UNION ALL
SELECT '2. STRIPE_CUSTOMERS',
       CASE WHEN COUNT(*) > 0 THEN '✅ Créé' ELSE '❌ Manquant' END
FROM stripe_customers
WHERE user_email = 'dominic.desy@icloud.com'
UNION ALL
SELECT '3. STRIPE_SUBSCRIPTIONS',
       CASE WHEN COUNT(*) > 0 THEN '✅ Actif' ELSE '❌ Manquant' END
FROM stripe_subscriptions
WHERE user_email = 'dominic.desy@icloud.com' AND status = 'active'
UNION ALL
SELECT '4. MONTHLY_USAGE_TRACKING',
       CASE WHEN COUNT(*) > 0 THEN '✅ Créé' ELSE '❌ Manquant' END
...
SELECT
    'user_billing_info' as table_name,
    user_email,
    plan_name,
    COALESCE(custom_monthly_quota::text, 'NULL') as quota,
