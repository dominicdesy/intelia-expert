# -*- coding: utf-8 -*-
#
# app/main.py - VERSION 4.1 COMPLETE AVEC TOUTES LES FONCTIONNALITES
# ✅ FUSION: Detection RAG corrigee + Toutes les fonctionnalites du backup
# 🚀 NOUVEAU: Systeme de cache statistiques automatique
# 🔧 CORRECTION CORS POUR CREDENTIALS: 'INCLUDE' - VERSION FINALE CONSERVEE
# 🎯 COMPLET: Tous les endpoints, middleware, monitoring, auth, etc.
# 🐳 NOUVEAU: Calcul memoire conteneur Docker precis
#
...
try:
    ...
    logger.info("🔧 Creation d'un router v1 temporaire...")
    
    # Creer un router temporaire et inclure manuellement les composants
    ...
    try:
        ...
        logger.info("✅ Expert router ajoute")
    except ImportError as e:
    ...
    try:
        ...
        logger.info("✅ Auth router ajoute avec montage corrige")
    ...
    try:
        ...
        logger.info("✅ Stats Fast router ajoute (endpoints ultra-rapides)")
    except ImportError as e:
        logger.warning(f"⚠️ Stats Fast router non disponible: {e}")

    # 🚀 Router stats_admin (optionnel)
    if STATS_CACHE_AVAILABLE:
        try:
            ...
            logger.info("✅ Stats Admin router ajoute (administration cache)")
        except ImportError as e:
    ...
    for router_info in [
        ...
        try:
            ...
            logger.info(f"✅ {tag.capitalize()} router ajoute")
        except ImportError as e:
            logger.warning(f"⚠️ {tag.capitalize()} router non disponible: {e}")
    
    # Monter le router temporaire
    app.include_router(temp_v1_router)
    logger.info("✅ Router v1 temporaire monte avec succes")

# === ENDPOINTS DE DEBUG RAG AMELIORES ===
@app.get("/rag/debug", tags=["Debug"])
async def rag_debug():
    """🔍 Debug des 3 RAG avec detection amelioree"""
    try:
        env_vars = {
...
async def debug_memory_comparison():
    """Compare les differentes methodes de calcul memoire"""
    try:
        import subprocess
        ...
        # 🐳 NOUVEAU: Calcul memoire conteneur precis
        container_percent = get_container_memory_percent()
        
        # free command
        try:
            free_result = subprocess.run(['free', '-h'], capture_output=True, text=True, timeout=5)
            free_lines = free_result.stdout.strip().split('\n')
            mem_line = free_lines[1].split() if len(free_lines) > 1 else []
        except:
            free_lines = []
            mem_line = []
        
        # Differents calculs
        psutil_used_percent = (memory.used / memory.total) * 100
        psutil_available_percent = (memory.available / memory.total) * 100
        
        return {
            "raw_data": {
                "psutil_total": memory.total,
                "psutil_used": memory.used,
                "psutil_available": memory.available,
                "psutil_percent": memory.percent,
                "free_command": free_result.stdout if 'free_result' in locals() else "N/A"
            },
            "calculations": {
                ...
                "psutil_percent": memory.percent,
                "calculated_used_percent": round(psutil_used_percent, 1),
                "calculated_available_percent": round(psutil_available_percent, 1),
                "container_memory_percent": container_percent,  # 🐳 NOUVEAU
                "cpu_percent": round(cpu_percent, 1)
            },
            "interpretations": {
                "if_psutil_percent_is_used": f"{memory.percent:.1f}% used = {100-memory.percent:.1f}% free",
                "if_psutil_percent_is_free": f"{memory.percent:.1f}% free = {100-memory.percent:.1f}% used",
                "container_real_usage": f"{container_percent}% of allocated container memory"  # 🐳 NOUVEAU
            },
            "digital_ocean_reference": {
                "backend_shown": "10% RAM usage",
                "expected_free": "~90%",
                "container_calculation": "Uses cgroup limits for accurate container memory"  # 🐳 NOUVEAU
            }
        }
    except Exception as e:
        return {"error": str(e)}

# === ENDPOINTS PRINCIPAUX (RESTAURES) ===
...
async def system_metrics():
    """📊 Metriques systeme pour monitoring externe"""
    try:
        ...
        # 🚀 Ajouter metriques cache si disponible
        if STATS_CACHE_AVAILABLE:
            base_metrics.update({
...
async def root():
    ...
    return {
        ...
        "deployment_version": "v4.1.0-complete-fusion-memory-fix"
    }
