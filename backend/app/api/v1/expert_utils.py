...
def log_question_to_analytics(
    ...
    """
    VERSION CORRIGÉE: Utilise directement LoggingManager pour sauvegarder en PostgreSQL
    Bypass complètement les fallbacks de logging.py
    """
    try:
        ...
        # Extraire les entités du payload
        entities = getattr(payload, 'entities', {}) or {}
        
        # Préparer les paramètres pour LoggingManager
        language = getattr(payload, 'lang', 'fr') or 'fr'
        
        # APPEL DIRECT au LoggingManager avec toutes nos corrections PostgreSQL
        manager.log_question_response(
            user_email=user_email,
            session_id=session_id,
            question_id=f"{session_id}_{int(time.time())}", # ID unique
            question=question,
            response_text=response_text[:5000],  # Limite selon notre schéma
            response_source=source,
            status=status,
            processing_time_ms=processing_time_ms,
            confidence=confidence_score,
            completeness_score=None,  # Pas utilisé pour l'instant
            language=language,
            intent=None,  # Pourrait être extrait du result si disponible
            entities=entities,  # Sera sérialisé avec Json() par notre correction
            error_info=error_info  # Sera traité correctement par nos corrections
        )
        
        logger.info(f"Question loggée dans PostgreSQL: {user_email or 'anonymous'}")
        
    except Exception as e:
        logger.error(f"Erreur log PostgreSQL direct: {e}")
        logger.error(f"Détails: user_email={user_email}, session_id={session_id}")
