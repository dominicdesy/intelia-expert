...
FIXED: Correction du système de logging PostgreSQL
"""
...
def log_question_to_analytics(
    ...
    """
    VERSION CORRIGÉE: Utilise le système de logging helpers corrigé
    Connecte le flux principal des questions au système PostgreSQL
    """
    try:
        # Utiliser le système helpers corrigé au lieu de l'import direct
        from app.api.v1.logging_helpers import log_question_to_analytics as helper_log
        
        # Appeler la fonction helper qui utilise le bon système de logging
        helper_log(
        ...
        logger.info("Question loggée dans PostgreSQL: %s", 
                   current_user.get('email') if current_user else 'anonymous')
        
    except Exception as e:
        logger.error("Erreur log PostgreSQL: %s", e)
        logger.error("Détails erreur logging: current_user=%s, payload=%s", 
...
async def log_analytics_async(
    ...
    try:
        ...
        await asyncio.to_thread(
            log_question_to_analytics,  # Utilise notre fonction corrigée
            current_user=current_user,
            payload=payload,
            result=result,
...
def validate_analytics_compatibility() -> Dict[str, Any]:
    ...
    try:
        from app.api.v1.logging_helpers import log_question_to_analytics as log_impl
        import inspect
        ...
        return {
            ...
            "uses_helpers": True,  # NOUVEAU: Indique qu'on utilise le système helpers
            "signature": str(sig)
        }
...
def get_expert_utils_status() -> Dict[str, Any]:
    ...
    return {
        "version": "memory_safe_v1.2_logging_fixed",  # Version mise à jour
        ...
        # NOUVEAU: Informations sur la correction logging
        "logging_fix": {
            "system_used": "logging_helpers",
            "direct_import_removed": True,
            "fixed_in_version": "v1.2_logging_fixed",
            "expected_log_message": "Question loggée dans PostgreSQL"
        }
    }
