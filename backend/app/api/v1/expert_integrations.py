...
class IntegrationsManager:
    ...
    def _initialize_integrations(self):
        ...
        try:
            ...
            # Tentative d'import des fonctions sémantiques dynamiques (optionnelles)
            try:
                from app.api.v1.question_clarification_system import (
                    analyze_question_for_clarification_semantic_dynamic,
                    generate_dynamic_clarification_questions
                )
                self._clarification_functions.update({
                ...
                logger.info("✅ [Integrations] Fonctions mode sémantique dynamique importées")
            except ImportError:
                logger.info("ℹ️ [Integrations] Fonctions sémantiques dynamiques non disponibles, utilisation du fallback")
            ...
            logger.info("✅ [Integrations] Système de clarification amélioré importé (mode sémantique dynamique FORCÉ ACTIF)")
    ...
    def _init_logging(self):
        """Initialise le système de logging avec gestion des erreurs"""
        try:
            ...
            if logger_instance:
                ...
                self.logging_available = False
                logger.warning("⚠️ [Integrations] Logger instance is None")
        except ImportError as e:
            self.logging_available = False
    ...
    def _fallback_get_examples(self, language: str = "fr") -> List[str]:
    ...
    def _fallback_validate_questions(self, questions: List[str], language: str = "fr") -> Dict[str, Any]:
        ...
        invalid_questions = [q for q in questions if not isinstance(q, str) or len(q.strip()) <= 10]
    ...
    def is_enhanced_clarification_enabled(self) -> bool:
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur is_enhanced_clarification_enabled: {e}")
                return False
        return False
    ...
    def _generate_intelligent_generic_questions(self, question: str, language: str = "fr") -> List[str]:
    ...
    def _generate_basic_clarification_questions(self, question: str, language: str = "fr") -> List[str]:
    ...
    # 🆕 NOUVEAU: Analyse avec mode sémantique dynamique (métho de héritée)
    ...
    def generate_dynamic_clarification_questions(self, question: str, language: str = "fr") -> List[str]:
    ...
    async def check_for_reprocessing_after_clarification(self, **kwargs):
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur check_for_reprocessing: {e}")
                return None
        return None
        ...
        if self.enhanced_clarification_available:
            ...
            if func:
                try:
                    ...
                    logger.error(f"❌ [Integrations] Erreur récupération stats: {e}")
                    base_stats = {}
    ...
    def add_message_to_conversation(self, **kwargs):
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur add_message_to_conversation: {e}")
                return None
        return None
    ...
    def get_conversation_memory_stats(self):
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur get_conversation_memory_stats: {e}")
                return {}
        return {}
    ...
    def is_agricultural_validation_enabled(self) -> bool:
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur is_agricultural_validation_enabled: {e}")
                return False
        return False
    ...
    def get_agricultural_validator_stats(self):
        ...
        if func:
            try:
                ...
                logger.error(f"❌ [Integrations] Erreur get_agricultural_validator_stats: {e}")
                return {}
        return {}
    ...
    async def update_feedback(self, conversation_id: str, rating_numeric: int) -> bool:
        ...
        try:
            ...
            # Fallback vers SQL direct si sqlite3 disponible
            try:
                import sqlite3
                ...
                logger.error(f"❌ [Integrations] Erreur fallback SQL: {e}")
                return False
    ...
    def build_contextualization_prompt(self, question: str, language: str = "fr") -> str:
        ...
        if func:
            try:
                ...
                logger.warning(f"⚠️ [Integrations] Erreur build_contextualization_prompt: {e}")
        
        # ✅ Fallback toujours disponible
        return self._fallback_build_contextualization_prompt(question, language)
    
    def get_dynamic_clarification_examples(self, language: str = "fr") -> List[str]:
        ...
        if func:
            try:
                ...
                logger.warning(f"⚠️ [Integrations] Erreur get_dynamic_clarification_examples: {e}")
        
        # ✅ Fallback toujours disponible
        return self._fallback_get_examples(language)
    
    def validate_dynamic_questions(self, questions: List[str], language: str = "fr") -> Dict[str, Any]:
        ...
        if func:
            try:
                ...
                logger.warning(f"⚠️ [Integrations] Erreur validate_dynamic_questions: {e}")
