...
# ---------- DialogueManager (import tolérant) ----------
# Objectif : ne JAMAIS casser le boot si un import en cascade échoue.
try:
    from .pipeline.dialogue_manager import DialogueManager as _RealDialogueManager  # type: ignore
    ...
    _RealDialogueManager = None  # type: ignore

class _FallbackDialogueManager:
...
def get_dialogue_manager():
    """
    Retourne l'instance réelle si disponible, sinon un fallback.
    On encapsule ici pour éviter les imports en cascade au boot.
    """
    try:
        if DIALOGUE_MANAGER_AVAILABLE and _RealDialogueManager is not None:
            return _RealDialogueManager()
        raise RuntimeError("Real DialogueManager unavailable")
    except Exception as e:
        logger.error(f"❌ Erreur création DialogueManager: {e}")
        return _FallbackDialogueManager()
