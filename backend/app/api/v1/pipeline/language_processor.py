...
def finalize_response_with_language(response: Dict[str, Any], question: str, effective_language: str, detected_language: str, force_conversation_language: bool = True) -> Dict[str, Any]:
    """
    🔧 VERSION CORRIGÉE: Helper pour appliquer l'adaptation linguistique à toute réponse finale.
    CORRECTION PRINCIPALE: Force l'adaptation même pour les réponses non-OpenAI fallback.
    """
    ...
    # 🔧 CORRECTION: Si français, pas de traitement supplémentaire nécessaire
    if effective_language == "fr":
        return response
    
    # 🔧 CORRECTION PRINCIPALE: Adapter le texte pour TOUTES les sources (sauf exceptions spécifiques)
...
def get_language_processing_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "version": "memory_safe_v1.0_language_fix",
