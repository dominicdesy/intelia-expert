# -*- coding: utf-8 -*-
"""
Dialogue orchestration - VERSION REFACTORISÉE CORRIGÉE + PERSISTANCE + FIX LANGUE + CONFIDENCE UNIFIÉ
...
- 🎯 NOUVEAU: Intégration du système de confidence score unifié
"""
...
# ========== 🎯 NOUVEAU: IMPORT SYSTÈME D'ANALYSE DES INTENTIONS ==========
try:
...
# ---------------------------------------------------------------------------
# 🎯 NOUVELLES FONCTIONS POUR INTÉGRATION CONFIDENCE UNIFIÉ
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FONCTION PRINCIPALE HANDLE - VERSION AVEC CONFIDENCE UNIFIÉ
# ---------------------------------------------------------------------------
...
def handle(
    ...
    # 🎯 NOUVEAU: Validation result pour confidence
    validation_result: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
    Fonction principale de traitement des questions - VERSION AVEC CONFIDENCE UNIFIÉ
    
    CONSERVE: Toute la logique métier originale
    NOUVEAU: Calcul automatique du confidence score unifié pour chaque réponse
    """
    try:
        ...
        if OPENAI_COT_AVAILABLE and should_use_cot_analysis(intent, entities, question):
            ...
            if cot_result:
                ...
                # 🎯 NOUVEAU: Calculer confidence unifié
                ...
                additional_context = {
                    ...
                    "confidence_score": final_response.get("confidence", {}).get("score")  # 🎯 NOUVEAU
                }
        ...
        # 🎯 NOUVEAU: Même en cas d'erreur, appliquer confidence unifié
        try:
            response = _apply_unified_confidence(response, None, None, validation_result)
        except Exception:
            response["confidence"] = {"score": 10.0, "level": "very_low", "explanation": "Erreur système"}
...
# ---------------------------------------------------------------------------
# FONCTIONS DE STATUT UNIFIÉES (code original conservé + enrichissements)
# ---------------------------------------------------------------------------
...
def get_fallback_status() -> Dict[str, Any]:
    ...
    unified_status = {
        ...
        "unified_confidence_system": "integrated",  # 🎯 NOUVEAU
...
def test_enhanced_pipeline() -> Dict[str, Any]:
    """
    Test complet du pipeline refactorisé + persistance + fix langue + modularité + confidence unifié
    """
    try:
        results = {}
        ...
        results["basic_status"] = {
            ...
            "perfstore_available": PERF_AVAILABLE,
            "postgres_available": POSTGRES_AVAILABLE,
            "confidence_system_available": UNIFIED_CONFIDENCE_AVAILABLE  # 🎯 NOUVEAU
        }
        
        # 🎯 NOUVEAU: Test système de confidence unifié
        try:
        ...
        try:
            ...
            persistence_success = _persist_conversation(
                ...
                language="fr",
                user_id="test_user",
                additional_context={"confidence_score": 85.0}  # 🎯 NOUVEAU
            )
        ...
        # Tests modules spécialisés (code original conservé)
        try:
        ...
        # 🔧 Test du fix langue conversationnelle (code original conservé)
        try:
        ...
        return {
            "status": "success",
            "message": "Pipeline refactorisé + persistance + fix langue + modularité + confidence unifié testé avec succès",
            "fixes_applied": [
                ...
                "système_confidence_unifié"  # 🎯 NOUVEAU
            ],
            "feature_flags": {
                "PERSIST_CONVERSATIONS": PERSIST_CONVERSATIONS,
                "CLEAR_CONTEXT_AFTER_ASK": CLEAR_CONTEXT_AFTER_ASK,
                "ENABLE_AUTO_LANGUAGE_DETECTION": str(os.getenv("ENABLE_AUTO_LANGUAGE_DETECTION", "true")).lower() in ("1", "true", "yes", "on"),
                "DEBUG_CONFIDENCE": str(os.getenv("DEBUG_CONFIDENCE", "false")).lower() in ("1", "true", "yes", "on")  # 🎯 NOUVEAU
            },
        ...
        return {
            "status": "error",
            "message": f"Échec test pipeline refactorisé + persistance + fix langue + modularité + confidence unifié: {str(e)}",
            "error_type": type(e).__name__
        }

# 🎯 NOUVELLES FONCTIONS DE TEST POUR CONFIDENCE UNIFIÉ

def test_confidence_integration() -> Dict[str, Any]:
    """
    Test spécifique de l'intégration du système de confidence unifié
    """
    try:
        if not UNIFIED_CONFIDENCE_AVAILABLE:
            return {
        ...
        # Test avec différents types de réponses
        test_cases = [
            {
