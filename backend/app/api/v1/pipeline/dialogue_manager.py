# -*- coding: utf-8 -*-
"""
Dialogue orchestration - VERSION HYBRIDE AVEC MÉMOIRE CONVERSATIONNELLE + FALLBACK OPENAI + MULTILINGUE UNIVERSEL
...
try:
    ...
    logger.info("✅ OpenAI fallback disponible pour réponses manquantes")
except ImportError as e:
    logger.warning(f"⚠️ OpenAI fallback indisponible: {e}")
...
# ---------------------------------------------------------------------------
# NOUVEAU: Détection de langue universelle et post-processing multilingue
# ---------------------------------------------------------------------------
...
def _finalize_response_with_language(response: Dict[str, Any], question: str, effective_language: str, detected_language: str) -> Dict[str, Any]:
    ...
    if effective_language == "fr":
        # Ajouter les métadonnées de langue même pour le français
        ...
        return response  # Pas de traitement nécessaire
...
# ---------------------------------------------------------------------------
# NOUVEAU: Fonctions de fallback OpenAI - CODE ORIGINAL CONSERVÉ
# ---------------------------------------------------------------------------
...
def _generate_openai_fallback_response(question: str, entities: Dict[str, Any], intent: Intention, rag_context: str = "") -> Dict[str, Any]:
    """
    Génère une réponse via OpenAI quand le RAG échoue
    MODIFIÉ: Le prompt reste en français, mais OpenAI détectera automatiquement la langue de la question
    """
    if not OPENAI_FALLBACK_AVAILABLE:
        return None
    ...
    try:
        ...
        # MODIFIÉ: Prompt adaptatif qui laisse OpenAI détecter la langue
...
Réponse professionnelle dans la langue de la question :"""
...
# ---------------------------------------------------------------------------
# RAG avec fallback OpenAI - CODE ORIGINAL CONSERVÉ
# ---------------------------------------------------------------------------

def _rag_answer_with_fallback(question: str, k: int = 5, entities: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Version améliorée de _rag_answer avec fallback OpenAI
    """
...
# ---------------------------------------------------------------------------
# Fonction de statut pour monitoring - CODE ORIGINAL CONSERVÉ
# ---------------------------------------------------------------------------

def get_fallback_status() -> Dict[str, Any]:
    """
    Retourne le statut du système de fallback OpenAI
    """
    status = {
...
# ---------------------------------------------------------------------------
# Entrée principale AVEC MÉMOIRE CONVERSATIONNELLE + FALLBACK OPENAI + MULTILINGUE
# ---------------------------------------------------------------------------
...
def handle(
    ...
    """
    Fonction principale de traitement des questions - VERSION HYBRIDE AVEC MÉMOIRE CONVERSATIONNELLE + FALLBACK OPENAI + MULTILINGUE UNIVERSEL
    """
    try:
        ...
        # =================================================================
        # NOUVEAU: DÉTECTION DE LANGUE AUTOMATIQUE
        # =================================================================
        ...
        # Étape 5: RAG complet avec fallback OpenAI
        logger.info("📚 RAG via RAGRetriever avec fallback OpenAI")
        
        # Passer l'intent dans les entities pour le fallback
        entities_with_intent = dict(entities)
        entities_with_intent["_intent"] = intent
        
        rag = _rag_answer_with_fallback(question, k=5, entities=entities_with_intent)
        rag_text = _final_sanitize(rag.get("text", ""))
        
        # Synthèse uniquement si ce n'est pas déjà un fallback OpenAI
        if rag.get("source") != "openai_fallback":
