...
class PostgresMemory:
    ...
    def __init__(self, dsn=None):
        ...
        if ASYNC_AVAILABLE:
            ...
            self.logger.warning(
                "⚠️ Support async indisponible (asyncpg manquant). Fonctionnement en mode sync uniquement."
            )
    ...
    async def _get_async_pool(self):
        ...
        if not ASYNC_AVAILABLE:
            raise RuntimeError(
                "asyncpg n'est pas installé. Installez-le avec: pip install asyncpg"
            )
        ...
        if not self._pool_initialized:
            try:
                ...
                self.logger.error(
                    f"❌ Erreur lors de l'initialisation du pool async: {e}"
                )
                raise
        ...
        try:
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    if not table_exists:
                        self.logger.info(
                            "🔧 Table conversation_memory n'existe pas, création en cours..."
                        )
                        ...
                        conn.commit()
                        self.logger.info(
                            "✅ Table conversation_memory créée avec succès avec indexes et triggers"
                        )
            ...
            self.logger.error(
                f"❌ Erreur lors de la vérification/création de la table: {e}"
            )
            raise
        ...
        try:
            ...
            for column_name, column_def in required_columns.items():
                if column_name not in existing_columns:
                    ...
                    cur.execute(
                        f"ALTER TABLE conversation_memory ADD COLUMN {column_name} {column_def}"
                    )
            ...
            if context_type and context_type[0] not in ["jsonb", "json"]:
                self.logger.warning(
                    "⚠️ La colonne 'context' n'est pas de type JSON/JSONB, conversion recommandée"
                )
            ...
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"⚠️ Erreur de parsing JSON pour session {session_id}: {e}"
                        )
                        return {}

        except psycopg2.Error as e:
            self.logger.error(
                f"❌ Erreur PostgreSQL lors de la récupération de session {session_id}: {e}"
            )
            return {}
        except Exception as e:
            self.logger.error(
                f"❌ Erreur inattendue lors de la récupération de session {session_id}: {e}"
            )
            return {}
            ...
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"⚠️ Fallback vers JSON string pour session {session_id}: {e}"
                        )
                    ...
                    conn.commit()
                    self.logger.debug(
                        f"✅ Contexte mis à jour pour session {session_id}"
                    )

        except psycopg2.Error as e:
            self.logger.error(
                f"❌ Erreur PostgreSQL lors de la mise à jour de session {session_id}: {e}"
            )
            raise
        except Exception as e:
            self.logger.error(
                f"❌ Erreur inattendue lors de la mise à jour de session {session_id}: {e}"
            )
            raise
    ...
    def clear(self, session_id):
        ...
        try:
            ...
            self.logger.error(
                f"❌ Erreur PostgreSQL lors de la suppression de session {session_id}: {e}"
            )
            raise
        except Exception as e:
            self.logger.error(
                f"❌ Erreur inattendue lors de la suppression de session {session_id}: {e}"
            )
            raise
    ...
    async def get_async(self, session_id: str) -> Dict[str, Any]:
        ...
        if not ASYNC_AVAILABLE:
            self.logger.warning(
                "⚠️ Méthode async appelée mais asyncpg indisponible. Utilisation de la version sync."
            )
        ...
        try:
            ...
            self.logger.error(
                f"❌ Erreur async lors de la récupération de session {session_id}: {e}"
            )
            return {}
    ...
    async def update_async(self, session_id: str, context: dict, user_id: str = None):
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                self.logger.debug(
                    f"✅ Contexte mis à jour (async) pour session {session_id}"
                )

        except Exception as e:
            self.logger.error(
                f"❌ Erreur async lors de la mise à jour de session {session_id}: {e}"
            )
            raise
        ...
        try:
            ...
            async with pool.acquire() as conn:
                result = await conn.execute(
                    "DELETE FROM conversation_memory WHERE session_id = $1", session_id
                )
            ...
            self.logger.error(
                f"❌ Erreur async lors de la suppression de session {session_id}: {e}"
            )
            raise

    async def save_context_async(
        self, session_id: str, inputs: dict, outputs: dict, user_id: str = None
    ):
        """
        ...
        if not ASYNC_AVAILABLE:
            self.logger.warning(
                "⚠️ save_context_async appelée mais asyncpg indisponible. Utilisation de fallback sync."
            )
            ...
            current_context["history"].append(
                {"inputs": inputs, "outputs": outputs, "timestamp": time.time()}
            )
        ...
        try:
            ...
            self.logger.debug(
                f"✅ Contexte sauvegardé (async) pour session {session_id}"
            )

        except Exception as e:
            self.logger.error(
                f"❌ Erreur lors de la sauvegarde async du contexte {session_id}: {e}"
            )
            raise

    async def persist_conversation_async(
        self, session_id: str, conversation_data: dict
    ):
        """
        ...
        if not ASYNC_AVAILABLE:
            self.logger.warning(
                "⚠️ persist_conversation_async appelée mais asyncpg indisponible."
            )
            return
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                self.logger.debug(
                    f"✅ Conversation persistée (async) pour session {session_id}"
                )

        except Exception as e:
            self.logger.error(
                f"❌ Erreur lors de la persistance async de conversation {session_id}: {e}"
            )
            raise
        ...
        try:
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    ...
                    if rows_deleted > 0:
                        self.logger.info(
                            f"🧹 Nettoyage: {rows_deleted} sessions anciennes supprimées"
                        )
            ...
            self.logger.error(
                f"❌ Erreur lors du nettoyage des anciennes sessions: {e}"
            )
            return 0
    ...
    async def cleanup_old_sessions_async(self, days_old=7):
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                if rows_deleted > 0:
                    self.logger.info(
                        f"🧹 Nettoyage async: {rows_deleted} sessions anciennes supprimées"
                    )
            ...
            self.logger.error(
                f"❌ Erreur lors du nettoyage async des anciennes sessions: {e}"
            )
            return 0
    ...
    async def get_stats_async(self):
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                if row:
                    return {
                        ...
                        "most_recent_activity": (
                            row["most_recent_activity"].isoformat()
                            if row["most_recent_activity"]
                            else None
                        ),
                    }
            ...
            self.logger.error(
                f"❌ Erreur lors de la récupération async des statistiques: {e}"
            )
            return {"error": str(e)}
    ...
    def __del__(self):
        ...
        if ASYNC_AVAILABLE and self._async_pool and not self._async_pool.is_closed():
            try:
                ...
                # Ignore si pas de loop disponible ou autre erreur d'event loop
                pass
