...
- ✅ MODIFIÉ: Support nouvelle fonction complete() sécurisée
"""
...
try:
    ...
    logger.info("✅ OpenAI utils sécurisés importés pour RAG engine")
except ImportError as e:
    logger.warning(f"⚠️ OpenAI utils non disponibles dans RAG engine: {e}")
...
# ========== NOUVELLE FONCTION ENHANCED SYNTHESIS ==========
def _enhanced_rag_synthesis(question: str, raw_content: str, entities: Dict[str, Any], 
                           complexity_info: Optional[Dict[str, Any]] = None) -> str:
    """
    Synthèse RAG améliorée utilisant la fonction complete() sécurisée
    """
    if not OPENAI_UTILS_AVAILABLE or not complete:
        logger.warning("⚠️ OpenAI utils non disponibles, retour contenu brut")
        return raw_content[:500] + "..." if len(raw_content) > 500 else raw_content
    
    try:
        # Construire le prompt de synthèse
        prompt_parts = [
            "Tu es un expert en aviculture. Synthétise les informations suivantes de manière professionnelle et précise.",
            f"QUESTION: {question}",
        ]
        
        # Ajouter contexte des entités si disponible
        if entities:
            relevant_entities = []
            for key, value in entities.items():
                if value is not None and key not in ['_intent']:
                    relevant_entities.append(f"{key}: {value}")
            if relevant_entities:
                prompt_parts.append(f"CONTEXTE: {', '.join(relevant_entities)}")
        
        # Ajouter instructions spécialisées si complexité détectée
        if complexity_info:
            intent = entities.get('_intent') if entities else None
            complexity_level = complexity_info.get("level", "simple")
            if complexity_level != "simple" and intent:
                instruction = _get_enhanced_rag_instruction(intent, complexity_level)
                prompt_parts.append(f"INSTRUCTIONS: {instruction}")
        
        prompt_parts.extend([
            f"INFORMATIONS TECHNIQUES:\n{raw_content}",
            "Synthétise une réponse claire et actionnable en français."
        ])
        
        synthesis_prompt = "\n\n".join(prompt_parts)
        
        # ✅ UTILISATION DE LA FONCTION COMPLETE SÉCURISÉE
        logger.debug("🔒 Utilisation complete() sécurisé pour synthèse RAG")
        ...
        if synthesized and len(synthesized.strip()) > 10:
            ...
            logger.warning("⚠️ Synthèse vide, retour contenu original")
            return raw_content[:500] + "..." if len(raw_content) > 500 else raw_content
            
    except Exception as e:
        logger.error(f"❌ Erreur lors de la synthèse RAG: {e}")
        return raw_content[:500] + "..." if len(raw_content) > 500 else raw_content
...
def answer_with_rag(question: str, entities: Dict[str, Any], intent=None, 
    ...
    ✅ MODIFIÉ: Utilise la fonction complete() sécurisée pour synthèse
    """
        ...
        if use_multihop:
            ...
            def search_fn(q):
                ...
                if docstore and scored:
                    ...
                    # ✅ UTILISATION DE LA SYNTHÈSE SÉCURISÉE
                    answer_text = _enhanced_rag_synthesis(q, _synthesize_answer(top_ids, docstore), entities, complexity_info)
                    return {
                        "text": answer_text,
            ...
            # Mode standard (code original adapté avec synthèse sécurisée)
            ...
            if docstore is not None and scored is not None and len(docstore) > 0:
                ...
                sources = _format_sources(top_ids, docstore)
                # ✅ UTILISATION DE LA SYNTHÈSE SÉCURISÉE
                raw_answer = _synthesize_answer(top_ids, docstore)
                answer_text = _enhanced_rag_synthesis(question, raw_answer, entities, complexity_info)
    ...
    if use_multihop:
        ...
        def search_fn(q):
            try:
                ...
                # ✅ UTILISATION DE LA SYNTHÈSE SÉCURISÉE
                raw_answer = _synthesize_answer(top_ids, docstore)
                answer_text = _enhanced_rag_synthesis(q, raw_answer, entities, complexity_info)
                return {
                    "text": answer_text,
                    "documents_used": sources
                }
        ...
        # Mode standard FAISS avec synthèse sécurisée
...
def get_rag_capabilities() -> Dict[str, Any]:
    ...
    return {
        ...
        "secure_synthesis": OPENAI_UTILS_AVAILABLE,
...
def test_rag_multihop(question: str = "Test multihop", entities: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "secure_synthesis_used": OPENAI_UTILS_AVAILABLE
        }
...
def test_secure_synthesis(question: str = "Test synthèse", raw_content: str = "Contenu test") -> Dict[str, Any]:
    """
    Test de la synthèse sécurisée
    """
    test_entities = {"species": "broiler", "line": "ross308", "age_days": 21}
    
    try:
        result = _enhanced_rag_synthesis(question, raw_content, test_entities)
        
        return {
            "status": "success",
            "openai_utils_available": OPENAI_UTILS_AVAILABLE,
            "synthesis_successful": bool(result and len(result.strip()) > 10),
            "original_length": len(raw_content),
            "synthesized_length": len(result) if result else 0
        }
