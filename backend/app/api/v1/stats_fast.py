...
try:
    from app.utils.auth import verify_super_admin_token
    AUTH_AVAILABLE = True
...
try:
    from app.stats.stats_cache import get_stats_cache
        ...
        if response.status_code == 200:
            ...
            system_health = {
                ...
                "features_enabled": {
                    ...
                    "openai_fallback": health_data.get("openai_configured", True)
                }
            }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            costs_data = {
                ...
                "total_tokens": billing_data.get("total_input_tokens", 0) + billing_data.get("total_output_tokens", 0),
                "requests": billing_data.get("total_requests", 0),
                "models_usage": billing_data.get("models_usage", {})
            }
            
            logger.info(f"Billing OpenAI intégré: ${costs_data['total_cost']:.2f}")
            return costs_data
    ...
    try:
        ...
        with psycopg2.connect(dsn, connect_timeout=10) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                billing_data = {
                    "plans": {
                        "free": {"user_count": user_count, "revenue": 0}
                    },
                    "total_revenue": 0.0,
                    "top_users": top_users
                }
...
async def get_dashboard_fast(
    current_user: dict = Depends(verify_super_admin_token) if AUTH_AVAILABLE else None
) -> Dict[str, Any]:
    ...
    try:
        ...
        system_health_data, usage_stats, performance_stats, billing_plans, openai_costs = await asyncio.gather(
        ...
        if isinstance(system_health_data, Exception):
            ...
            system_health_data = {"uptime_hours": 0, "error_rate": 0, "rag_status": {"global": False}}
        ...
        if isinstance(billing_plans, Exception):
            ...
            billing_plans = {"plans": {"free": {"user_count": 0, "revenue": 0}}, "total_revenue": 0.0}
        ...
        formatted_response = {
            ...
            "system_stats": {
                "system_health": {
                    ...
                    "error_rate": system_health_data.get("error_rate", 0),
                    "rag_status": system_health_data.get("rag_status", {
                        "global": True, "broiler": True, "layer": True
                    })
                },
                "billing_stats": {
            ...
            "performance_stats": {
                ...
                "median_response_time": performance_stats.get("median_response_time", 0.0),
    ...
    try:
        ...
        with psycopg2.connect(dsn, connect_timeout=15) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                top_accepted = [inv for inv in top_inviters if inv['invitations_accepted'] > 0][:5]
...
async def cache_health() -> Dict[str, Any]:
    ...
    try:
        ...
        health_status = {
            ...
            "integrations": {
                "health_endpoint": "ok" if health_data.get("uptime_hours", 0) > 0 else "degraded",
                "billing_endpoint": "ok" if openai_data.get("success", False) else "degraded"
            },
            "system_stats": {
                "uptime_hours": health_data.get("uptime_hours", 0),
