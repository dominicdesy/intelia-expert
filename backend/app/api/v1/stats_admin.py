...
print(f"Composants disponibles: AUTH={AUTH_AVAILABLE}, CACHE={CACHE_AVAILABLE}, UPDATER={UPDATER_AVAILABLE}")
print("=" * 80)

logger.info("🚀 STATS_ADMIN VERSION SIMPLE V1.0 chargé")
logger.info(f"✅ Imports: AUTH={AUTH_AVAILABLE}, CACHE={CACHE_AVAILABLE}, UPDATER={UPDATER_AVAILABLE}")
...
async def clear_cache(
    ...
    try:
        ...
        logger.info(f"Cache vidé par {current_user.get('email')}: {deleted_count} entrées")
...
async def test_components(
    ...
    results = {
        "auth": {"available": AUTH_AVAILABLE, "status": "ok" if AUTH_AVAILABLE else "missing"},
    ...
    if UPDATER_AVAILABLE:
        try:
            ...
            results["updater"].update({
                ...
                "total_questions": stats.get("usageStats", {}).get("total_questions", 0) if stats else 0
            })
