...
def complete(messages, model: str, temperature: float = 0.2, max_tokens: int = 800,
    ...
    # Retry si température non supportée
    try:
        err_msg_full = err.get("message", "") if isinstance(err, dict) else (r.text or "")
    except Exception:
        err_msg_full = r.text or ""
    if _is_temp_unsupported_error(err_msg_full):
...
def complete_text(prompt: str, temperature: float = 0.2, max_tokens: Optional[int] = None, model: Optional[str] = None) -> str:
    ...
    try:
        ...
        response = complete(messages, model, safe_temp if safe_temp is not None else None, max_tokens)  # Force temperature=1.0
...
def safe_chat_completion(**kwargs) -> Any:
    ...
    try:
        ...
        try:
            ...
            # Retry si température non supportée par le modèle
            if _is_temp_unsupported_error(error_msg):
                logger.warning(f"[OpenAI] Température non supportée par {model} → retry sans 'temperature'")
                kwargs.pop('temperature', None)
                response = openai.chat.completions.create(**kwargs)
                error_msg = ""  # reset to avoid re-raising
        response = openai.chat.completions.create(**kwargs)
            else:
                raise
