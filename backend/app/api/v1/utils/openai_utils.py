...
_RESTRICTED_TEMP_MODELS = set(
    x.strip()
    for x in os.getenv("OPENAI_RESTRICTED_TEMP_MODELS", "").split(",")
    if x.strip()
)
...
def _is_temp_unsupported_error(msg: str) -> bool:
    ...
    return bool(
        re.search(
            r"temperature.+(unsupported|does\s*not\s*support|only\s*the\s*default)",
            str(msg),
            re.I,
        )
    )
...
def complete(
    ...
    if _is_temp_unsupported_error(err_msg):
        logger.warning(
            f"Température non supportée par {model} ? retry sans 'temperature'"
        )
...
def complete_text(
    ...
    if model.startswith("gpt-5"):
        logger.info(
            f"model={model}, max_tokens={max_tokens}, temp={safe_temp}, prompt_len={len(prompt)}"
        )
    ...
    if not content:
        ...
        logger.error(
            f"Réponse sans 'content'. Model: {model}, finish_reason: {finish_reason}, Aperçu: {str(response)[:500]}"
        )

        # Retry avec plus de tokens si coupé par la limite
        if finish_reason == "length" and max_tokens < 3000:
            logger.warning(
                f"Retry avec max_tokens augmenté: {max_tokens} -> {max_tokens * 2}"
            )
...
def _parse_cot_sections(raw: str) -> Dict[str, str]:
    ...
    for tag in ("<thinking>", "<analysis>", "<plan>", "<answer>", "<recommendations>"):
        name = tag.strip("<>")
        m = re.search(
            rf"{re.escape(tag)}(.*?){re.escape('</' + name + '>')}", raw, re.S | re.I
        )
        if m:
...
def synthesize_rag_content(
    question: str, raw_content: str, max_length: int = 300
) -> str:
    """
...
def generate_clarification_response(
    intent: str, missing_fields: List[str], general_info: str = ""
) -> str:
    """
...
Quelle est la performance attendue d'un poulet à 35 jours ?"""
        ...
        cot_result = complete_with_cot(
            prompt=test_cot_prompt, temperature=0.2, max_tokens=200, parse_cot=True
        )
        ...
        return {
            ...
            "simple_completion_test": {
                ...
                "response_preview": (
                    simple_test[:100] + "..." if len(simple_test) > 100 else simple_test
                ),
            },
            "cot_no_structure": {
                "success": True,
    ...
    try:
        ...
        return {
            ...
            "clarification_test": {
                "success": True,
                "response": (
                    clarification_test[:100] + "..."
                    if len(clarification_test) > 100
                    else clarification_test
                ),
            },
...
def get_openai_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "retry_config": {"max_retries": 2, "base_delay": 1.0},
