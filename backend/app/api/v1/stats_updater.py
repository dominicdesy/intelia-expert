...
class StatisticsUpdater:
    def __init__(self):
        ...
        logger.info(
            f"DSN configuré depuis DATABASE_URL (longueur: {len(self.dsn)} chars)"
        )
        logger.info(
            "Cette version corrige COMPLETEMENT le format de retour pour main.py"
        )
            ...
            with psycopg2.connect(self.dsn) as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if result and result["total_questions"] > 0:
                        logger.info(
                            f"Stats récupérées: {result['total_questions']} questions, {result['unique_users']} utilisateurs"
                        )
                        ...
                        return {
                            ...
                            "performanceStats": {
                                "avg_response_time": float(
                                    result["avg_response_time"] or 0
                                ),
                                "min_response_time": float(
                                    result["min_response_time"] or 0
                                ),
                                "max_response_time": float(
                                    result["max_response_time"] or 0
                                ),
                        ...
                        logger.warning(
                            "Aucune donnée trouvée dans user_questions_complete"
                        )
    ...
    def _get_empty_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "systemStats": {
                ...
                "billing_stats": {"plans_available": 0, "plan_names": []},
                "features_enabled": {
                    "analytics": False,
                    "billing": False,
            ...
            "billingStats": {"total_revenue": 0.0, "top_users": [], "plans": {}},
            "meta": {
                "collected_at": datetime.now().isoformat(),
    ...
    async def update_all_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Mise à jour terminée: {stats['usageStats']['total_questions']} questions"
            )
...
async def run_update_cycle() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Cycle terminé: {result.get('status')} - {result.get('successful_updates', 0)}/{result.get('total_updates', 0)} succès"
        )
        return result
