...
class StatisticsUpdater:
    ...
    def __init__(self):
        logger.info("StatisticsUpdater CORRIGÉ - Utilise directement user_questions_complete + invalidate_cache")
    ...
    async def update_all_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # CORRECTION CRITIQUE: invalidate_cache() au lieu de delete_cache()
            try:
                self.cache.invalidate_cache(key="dashboard:main")
                self.cache.invalidate_cache(key="dashboard:snapshot")
                logger.info("Cache dashboard invalidé avec invalidate_cache()")
    ...
    def _calculate_source_distribution(self, cur) -> Dict[str, int]:
        """Distribution des sources - utilise user_questions_complete directement"""
        source_distribution = {}
