                ...
                try:
                    ...
                    if server_stats and "error" not in server_stats:
                        ...
                        dashboard_data.update({
                            ...
                            "system_health": current_status.get("overall_health", "healthy")
                        })
    ...
    async def _update_server_performance_safe(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if UPDATER_CONFIG["SKIP_HEAVY_ANALYTICS"]:
                ...
                with psycopg2.connect(self.analytics.dsn) as conn:
                    with conn.cursor(cursor_factory=RealDictCursor) as cur:
                        ...
                        performance_data = {
                            "period_hours": 24,
                            "current_status": {
                                "overall_health": "healthy" if error_rate < 5 else ("warning" if error_rate < 15 else "degraded"),
                ...
                # Version complète conservée avec correction
                try:
                    from app.api.v1.logging import get_server_analytics
                    ...
                    if server_metrics and "error" not in server_metrics:
                        ...
                        if "overall_health" not in current_status:
                            current_status["overall_health"] = "healthy"
                        ...
                        performance_data = {
                            ...
                            "collected_at": datetime.now().isoformat(),
                            "source": "server_analytics_safe"
                        }
                    else:
                        raise Exception(server_metrics.get("error", "Données serveur indisponibles"))
                        
                except Exception as server_error:
                    logger.warning(f"Erreur analytics serveur, fallback local: {server_error}")
                    # Fallback vers version allégée récursive
            ...
            logger.info(f"Performance serveur collectée safe: {health_status}")
            return {
                "status": "success", 
