...
class ConceptDetector:
    ...
    def detect_concepts(self, query: str) -> Dict[str, float]:
        ...
        concept_scores = {
            "performance": self._score_concept_category(
                query_lower, self.performance_concepts
            ),
            "nutrition": self._score_concept_category(
                query_lower, self.nutrition_concepts
            ),
            "health": self._score_concept_category(query_lower, self.health_concepts),
            "management": self._score_concept_category(
                query_lower, self.management_concepts
            ),
            "species_specific": self._score_concept_category(
                query_lower, self.species_indicators
            ),
            "line_specific": self._score_concept_category(
                query_lower, self.line_indicators
            ),
        }
    ...
    def _score_concept_category(
        self, query: str, concept_dict: Dict[str, List[str]]
    ) -> float:
    ...
    def _detect_quantitative_intent(self, query: str) -> float:
        ...
        matches = sum(
            1 for pattern in quantitative_patterns if re.search(pattern, query)
        )
...
class ConceptRouter:
    ...
    def _determine_route(
        ...
        if (
            concepts["species_specific"] > 0.5
            and concepts["line_specific"] > 0.3
            and concepts["performance"] > 0.3
        ):
            return {
                "route": QueryRoute.PERF_STORE,
                "confidence": 0.75,
        ...
        if (
            ...
            or concepts["nutrition"] > 0.3
            or concepts["health"] > 0.3
        ):
            return {
    ...
    def _extract_filters(
        self, query: str, concepts: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        # Extraction âge
        age_match = re.search(
            r"(\d+)\s*(day|days|jour|jours|week|weeks|semaine)", query_lower
        )
        if age_match:
        ...
        for (
            metric_category,
            keywords_dict,
        ) in self.concept_detector.performance_concepts.items():
    ...
    def _generate_clarification(
        self, concepts: Dict[str, float], filters: Dict[str, Any]
    ) -> str:
        """
