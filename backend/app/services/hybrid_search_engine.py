...
class HybridSearchEngine:
    ...
    async def search(self, 
                    query: str, 
                    context: Optional[SearchContext] = None) -> SearchResult:
        """
        ...
        try:
            ...
            if query_intent.route.value == "perf_store":
                result = await self._search_performance_focused(query, query_intent, context)
            elif query_intent.route.value == "rag_vector":
                result = await self._search_contextual_focused(query, query_intent, context)
            elif query_intent.route.value == "hybrid":
                result = await self._search_hybrid_combined(query, query_intent, context)
            else:  # clarification
            ...
            logger.info(f"Recherche complétée: {query_intent.route.value}, confiance: {result.confidence:.2f}")
            return result
    ...
    async def _search_performance_focused(self, 
                                        query: str, 
                                        intent, 
                                        context: Optional[SearchContext]) -> SearchResult:
        """
    ...
    async def _search_contextual_focused(self, 
                                       query: str, 
                                       intent, 
                                       context: Optional[SearchContext]) -> SearchResult:
        """
    ...
    async def _search_hybrid_combined(self, 
                                    query: str, 
                                    intent, 
                                    context: Optional[SearchContext]) -> SearchResult:
        """
        ...
        return SearchResult(
            ...
            metadata={"route": "hybrid", "fusion_strategy": response.get("fusion_strategy")}
        )
    ...
    async def _handle_clarification(self, 
        ...
        return SearchResult(
            ...
            sources=[],
            metadata={"route": "clarification", "clarification_type": response.get("type")}
        )
    
    async def _create_performance_response(self, 
                                         query: str, 
                                         perf_result, 
        ...
        context_info = ""
        if rag_context:
            context_info = self._extract_contextual_insights(rag_context, intent.filters)
...
"""
    ...
    async def _create_contextual_response(self, 
                                        query: str, 
...
"""
    ...
    async def _create_hybrid_response(self, 
                                    query: str, 
                                    perf_result: SearchResult, 
    ...
    async def _create_clarification_response(self, 
                                           query: str, 
                                           intent, 
                                           context: Optional[SearchContext]) -> Dict[str, Any]:
        """
        ...
        if available_data["species_available"]:
            suggestions.append(f"Espèces disponibles: {', '.join(available_data['species_available'])}")
        if available_data["lines_available"]:
            suggestions.append(f"Lignées disponibles: {', '.join(available_data['lines_available'][:5])}")
    ...
    async def _get_related_performance_data(self, filters: Dict[str, Any]) -> Optional[pd.DataFrame]:
        """
    ...
    def _format_performance_data(self, df: pd.DataFrame, filters: Dict[str, Any]) -> str:
        """
        ...
        if available_columns:
            ...
            return f"Données disponibles: {len(df)} entrées, colonnes: {list(df.columns)}"
