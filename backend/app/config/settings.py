...
class Settings:
    ...
    def agricultural_validation_enabled(self) -> bool:
        ...
        return (
            self._get_secret("ENABLE_AGRICULTURAL_VALIDATION", "true").lower() == "true"
        )
    ...
    def is_agricultural_validation_configured(self) -> bool:
        ...
        return (
            self.agricultural_validation_enabled
            and 0 <= self.agricultural_validation_strictness <= 100
        )
    ...
    def is_clarification_system_configured(self) -> bool:
        ...
        return (
            self.clarification_system_enabled
            and self.is_openai_configured
            and 0.0 <= self.clarification_confidence_threshold <= 1.0
        )
    ...
    def is_conversation_memory_configured(self) -> bool:
        ...
        return (
            ...
            and self.conversation_max_messages > 0
            and self.conversation_expiry_hours > 0
        )
    ...
    def validate_configuration(self) -> tuple[bool, list]:
        ...
        if (
            self.agricultural_validation_enabled
            and not self.is_agricultural_validation_configured
        ):
            issues.append("Agricultural validation enabled but misconfigured")

        if (
            self.clarification_system_enabled
            and not self.is_clarification_system_configured
        ):
            issues.append("Clarification system enabled but misconfigured")

        if (
            self.conversation_memory_enabled
            and not self.is_conversation_memory_configured
        ):
            issues.append("Conversation memory enabled but misconfigured")

        # Vérifications de cohérence
        if (
            self.clarification_confidence_threshold < 0.1
            or self.clarification_confidence_threshold > 0.95
        ):
            issues.append(
                f"Clarification confidence threshold unusual: {self.clarification_confidence_threshold}"
            )

        if (
            self.agricultural_validation_strictness < 5.0
            or self.agricultural_validation_strictness > 50.0
        ):
            issues.append(
                f"Agricultural validation strictness unusual: {self.agricultural_validation_strictness}"
            )
