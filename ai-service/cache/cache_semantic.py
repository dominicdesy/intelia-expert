...
class SemanticCacheManager:
    ...
    def _load_semantic_config(self):
        ...
        self.ENABLE_SEMANTIC_CACHE = (
            os.getenv("CACHE_ENABLE_SEMANTIC", "true").lower() == "true"
        )
        self.ENABLE_FALLBACK_KEYS = (
            os.getenv("CACHE_ENABLE_FALLBACK", "true").lower() == "true"
        )
        self.ENABLE_SEMANTIC_FALLBACK = (
            os.getenv("CACHE_ENABLE_SEMANTIC_FALLBACK", "true").lower() == "true"
        )
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    with open(path, "r", encoding="utf-8") as f:
                        ...
                        logger.info(
                            f"Aliases chargés depuis {path}: {len(aliases)} catégories"
                        )
                        return aliases
    ...
    def _build_semantic_vocabulary(self) -> Set[str]:
        ...
        logger.info(
            f"Vocabulaire sémantique STRICT construit: {len(vocabulary)} termes"
        )
        return vocabulary
                ...
                for alias in aliases:
                    if alias and len(alias) >= 3:
                        ...
                        if re.search(rf"\b{alias_pattern}\b", result):
                            result = re.sub(
                                rf"\b{alias_pattern}\b", main_normalized, result
                            )
                            changes_made += 1

        if changes_made > 0:
            logger.debug(
                f"Aliases appliqués: '{text}' -> '{result}' ({changes_made} changements)"
            )
    ...
    def _extract_semantic_keywords_strict(self, text: str) -> Set[str]:
        ...
        age_pattern = re.search(
            r"(\d+)\s*(?:j|jours?|day|days?|semaines?|semaine|wk|w)", text_lower
        )
        age_detected = False
        if age_pattern:
        ...
        if not (line_detected and metric_detected):
            logger.debug(
                f"Cache sémantique rejeté (composants manquants): line={line_detected}, metric={metric_detected}"
            )
            return set()
        ...
        logger.debug(
            f"Keywords sémantiques STRICTS extraits: {all_keywords} de '{text[:50]}...'"
        )
        return all_keywords
    ...
    def _extract_semantic_fallback_keywords(self, text: str) -> Set[str]:
        ...
        has_line = any(
            "ross" in kw or "cobb" in kw or "hubbard" in kw for kw in fallback_keywords
        )
        has_metric = any(
            kw in ["fcr", "poids", "température", "mortalité"]
    ...
    def _validate_semantic_cache_eligibility(
        self, keywords: Set[str], original_text: str
    ) -> bool:
        ...
        if len(keywords) < self.SEMANTIC_MIN_KEYWORDS:
            logger.debug(
                f"Rejeté: keywords insuffisants ({len(keywords)} < {self.SEMANTIC_MIN_KEYWORDS})"
            )
            return False

        # Règle 2: Au moins une lignée ET une métrique
        has_line = any(
            "ross" in kw or "cobb" in kw or "hubbard" in kw for kw in keywords
        )
        has_metric = any(
            kw in ["fcr", "poids", "température", "mortalité"] for kw in keywords
        )

        if not (has_line and has_metric):
            logger.debug(
                f"Rejeté: composants manquants - line={has_line}, metric={has_metric}"
            )
            return False
        ...
        if not has_age:
            logger.debug("Rejeté: âge requis pour cache strict")
            return False
        ...
        has_specific_context = any(
            all(term in text_lower for term in context) for context in specific_contexts
        )

        if not has_specific_context:
            logger.debug("Rejeté: contexte non spécifique")
    ...
    def _generate_key(
        ...
        if isinstance(data, str):
            ...
            if (
                use_semantic
                and self.ENABLE_SEMANTIC_CACHE
                and prefix in ["response", "intent", "embedding"]
            ):
                ...
                if self._validate_semantic_cache_eligibility(keywords, data):
                    ...
                    hash_obj = hashlib.md5(
                        semantic_signature.encode("utf-8"), usedforsecurity=False
                    )
                    logger.debug(
            ...
            if fallback_semantic and self.ENABLE_SEMANTIC_FALLBACK:
                ...
                if len(fallback_keywords) >= 2:
                    ...
                    hash_obj = hashlib.md5(
                        fallback_signature.encode("utf-8"), usedforsecurity=False
                    )
                    logger.debug(
        ...
        if isinstance(original_data, str):
            ...
            if minimal_normalized != self._normalize_text_extended(original_data):
                simple_hash = hashlib.md5(
                    minimal_normalized.encode(), usedforsecurity=False
                ).hexdigest()
    ...
    async def set_embedding(self, text: str, embedding: List[float]):
        ...
        try:
            ...
            await self.core.client.setex(
                key, self.core.ttl_config["embeddings"], compressed
            )
            ...
            if self.ENABLE_SEMANTIC_CACHE:
                ...
                if self._validate_semantic_cache_eligibility(keywords, text):
                    semantic_key = self._generate_key(
                        "embedding", text, use_semantic=True
                    )
                    if semantic_key != key:
                        await self.core.client.setex(
                            semantic_key, self.core.ttl_config["embeddings"], compressed
                        )
                        logger.debug(
                            f"Cache SET (sémantique STRICT): embedding '{text[:30]}...' -> keywords: {list(keywords)}"
                        )

            logger.debug(
                f"Cache SET: embedding pour '{text[:30]}...' ({len(compressed)} bytes)"
            )
    ...
    async def get_response(
        ...
        try:
            ...
            if self.ENABLE_SEMANTIC_FALLBACK:
                fallback_semantic_key = self._generate_key(
                    "response", query, fallback_semantic=True
                )
            ...
            if self.ENABLE_FALLBACK_KEYS:
                ...
                for fallback_key in fallback_keys:
                    ...
                    if cached:
                        ...
                        logger.info(
                            f"Cache HIT (fallback traditionnel): '{query[:30]}...'"
                        )
                        return response
    ...
    async def set_response(
        ...
        try:
            ...
            key = self._generate_key("response", cache_data, use_semantic=False)
            await self.core.client.setex(
                key, self.core.ttl_config["responses"], response_bytes
            )
            ...
            if self.ENABLE_SEMANTIC_CACHE:
                ...
                if self._validate_semantic_cache_eligibility(keywords, query):
                    semantic_key = self._generate_key(
                        "response", query, use_semantic=True
                    )
                    if semantic_key != key:
                        await self.core.client.setex(
                            semantic_key,
                        ...
                        logger.debug(
                            f"Cache SET (sémantique STRICT): '{query[:30]}...' -> keywords: {list(keywords)}"
                        )
            ...
            if self.ENABLE_SEMANTIC_FALLBACK:
                ...
                if len(fallback_keywords) >= 2:
                    ...
                    if fallback_semantic_key not in [
                        key,
                        semantic_key if "semantic_key" in locals() else None,
                    ]:
                        await self.core.client.setex(
                        ...
                        logger.debug(
                            f"Cache SET (sémantique FALLBACK): '{query[:30]}...' -> keywords: {list(fallback_keywords)}"
                        )

            logger.debug(
                f"Cache SET: réponse '{query[:30]}...' ({len(response_bytes)} bytes)"
            )
    ...
    async def set_intent_result(self, query: str, intent_result: Any):
        ...
        try:
            ...
            key = self._generate_key("intent", query, use_semantic=False)
            await self.core.client.setex(
                key, self.core.ttl_config["intent_results"], compressed
            )
            ...
            if self.ENABLE_SEMANTIC_CACHE:
                ...
                if self._validate_semantic_cache_eligibility(keywords, query):
                    semantic_key = self._generate_key(
                        "intent", query, use_semantic=True
                    )
                    if semantic_key != key:
                        await self.core.client.setex(
                            semantic_key,
            ...
            logger.debug(
                f"Cache SET: analyse d'intention '{query[:30]}...' ({len(compressed)} bytes)"
            )
        ...
        try:
            ...
            if self.core.client:
                try:
                    ...
                    fallback_semantic_cached = await self.core.client.get(
                        fallback_semantic_key
                    )
            ...
            return {
                ...
                "is_semantic_eligible": self._validate_semantic_cache_eligibility(
                    keywords, original
                ),
                "validation": {
                    "has_line": any(
                        "ross" in kw or "cobb" in kw or "hubbard" in kw
                        for kw in keywords
                    ),
                    "has_metric": any(
                        kw in ["fcr", "poids", "température", "mortalité"]
                    ...
                    "has_age": any(
                        "j" in kw and kw != "_fallback_eligible" for kw in keywords
                    ),
            ...
            return {"error": str(e), "original_query": query, "extraction_failed": True}
