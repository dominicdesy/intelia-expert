...
class CodebaseAnalyzer:
    ...
    def __init__(self, root_dir: Path):
        ...
        self.defined_items: Dict[str, Set[str]] = defaultdict(
            set
        )  # file -> defined functions/classes
        self.used_items: Dict[str, Set[str]] = defaultdict(
            set
        )  # file -> used functions/classes
    ...
    def find_dead_code(self) -> List[Tuple[str, str, str]]:
        ...
        for file_path, defined in self.defined_items.items():
            for item in defined:
                ...
                if not is_used and not any(
                    pattern in item_name for pattern in skip_patterns
                ):
                ...
                if any(file.endswith(ext) for ext in config_extensions):
                    ...
                    if not is_referenced:
                        unused_configs.append(
                            (file_path, "Not referenced in any Python file")
                        )
    ...
    def generate_report(self, output_file: Path):
        ...
        report = {
            "summary": {
                ...
                "total_defined_items": sum(
                    len(items) for items in self.defined_items.values()
                ),
            },
            "unused_files": [
                {
                ...
                for path, reason in sorted(
                    unused_files, key=lambda x: x[0].stat().st_size, reverse=True
                )
            ],
            "dead_code": [
                {"file": file, "type": item_type, "name": name}
        ...
        print(
            f"📋 Unused config files found: {report['summary']['unused_config_files']}"
        )
        ...
        if unused_files:
            ...
            for path, reason in unused_files[:10]:
                ...
                print(
                    f"  - {path.relative_to(self.root_dir)} ({size_kb:.1f} KB) - {reason}"
                )
...
def main():
    ...
    if (
        report["summary"]["unused_files"] > 0
        or report["summary"]["dead_code_items"] > 10
    ):
