...
def view_codebase_analysis():
    ...
    report_file = (
        Path(__file__).parent.parent / "logs" / "codebase_analysis_report.json"
    )
...
def view_deep_optimization():
    ...
    report_file = (
        Path(__file__).parent.parent / "logs" / "deep_optimization_report.json"
    )
    ...
    print(
        f"Analysis duration: {summary.get('analysis_duration_seconds', 0):.1f} seconds"
    )
    ...
    if duplication.get("total_duplicates", 0) > 0:
        ...
        for dup in duplication.get("duplicate_blocks", [])[:3]:
            ...
            print(
                f"    Locations: {', '.join([loc['file'] for loc in dup['locations'][:3]])}"
            )
    ...
    if type_hints:
        ...
        print(
            f"  Typed functions: {type_hints.get('typed_functions', 0)}/{type_hints.get('total_functions', 0)}"
        )
    ...
    if docs:
        ...
        print(
            f"  Documented items: {docs.get('documented_items', 0)}/{docs.get('total_items', 0)}"
        )
    ...
    if long_funcs.get("total_long_functions", 0) > 0:
        ...
        for func in long_funcs.get("functions_over_50_lines", [])[:5]:
            print(
                f"  - {func['file']}:{func['line']} - {func['function']} ({func['length']} lines)"
            )
...
def main():
    ...
    print("=" * 80)
    print(
        f"LLM OPTIMIZATION ANALYSIS RESULTS - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("=" * 80)
