...
def analyze_bandit_report(report_path="bandit_report.json"):
    ...
    print("-" * 80)
    sorted_by_count = sorted(by_test_id.items(), key=lambda x: len(x[1]), reverse=True)[
        :10
    ]
    ...
    for test_id, issues in sorted_by_count:
        ...
        print(
            f"   Sévérité: {severity} | Confiance: {confidence} | Occurences: {count}"
        )
    ...
    if quick_wins:
        ...
        for test_id, issues in sorted(
            quick_wins_by_type.items(), key=lambda x: len(x[1]), reverse=True
        ):
            first = issues[0]
